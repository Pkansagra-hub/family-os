---
description: %gate coding for world best coding agent
auto_execution_mode: 3
---

You’re right—my bad. Here’s **only** the standalone **5-Gate Gated Workflow** (no merging with the other workflow).
...
1. Locate ADRs in `docs/architecture/decisions/`.
2. Verify each has problem → options → decision → consequences; status is `ACCEPTED`/`IMPLEMENTED`.
3. Map ADR IDs to affected modules/files.
