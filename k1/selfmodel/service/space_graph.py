"""``SpaceGraphService`` — projects ``F`` (space graph) for a single actor.

Issue M1.E1.I2.

Returns a ``SpaceGraphSnapshot`` containing:

- ``members``: only those the actor is permitted to see per
  ``C.visibility_rules`` (Empty-Set Invariant E2 enforced upstream by
  the composer; this service surfaces the *raw* space snapshot so the
...
- ``routines``: full set (routine references are space-public by
...
class SpaceViewResult:
    """Composite result: space snapshot + freshness for the space row."""
    ...
    def __repr__(self) -> str:  # pragma: no cover - debug aid
        return (
            f"SpaceViewResult(space_id={self.snapshot.space_id!r}, "
...
class SpaceGraphService:
    """Project the space-graph snapshot for a viewing actor.
    ...
    # Read
    # ------------------------------------------------------------------
    def get_view(self, actor_id: str, space_id: str) -> SpaceViewResult:
        """Return the space-graph view projected for ``actor_id``.

        If the space has no row in the projection store, returns
        an empty snapshot (the protocol contract for
        ``ISelfSpacePort.get_space_view`` is "always returns a
        snapshot, never None"). Single-actor spaces therefore
        ...
        raw, result = self._store.read_space(space_id)
        if raw is None:
            empty = SpaceGraphSnapshot(
                space_id=space_id,
                composed_at_ms=_now_ms(),
            )
            logger.debug(
                "space_graph.get_view: no space row for %s, returning empty snapshot",
                space_id,
            )
            return SpaceViewResult(empty, freshness=result.freshness)
        ...
        return SpaceViewResult(projected, freshness=result.freshness)

    def get_member(self, space_id: str, member_id: str) -> ActorRef | None:
        """Helper for the composer: look up a single member ref."""
        raw, _ = self._store.read_space(space_id)
        if raw is None:
            return None
    ...
    def freshness(self, space_id: str) -> ProjectionFreshness:
        return self._store.freshness(f"space:{space_id}")

    # ------------------------------------------------------------------
    # Write (M7.E2.I3) — typed space routine writer.
    # ------------------------------------------------------------------
    def update_routines(
        self,
        space_id: str,
        ...
        """Replace the space's routines with ``routines``.

        Members and relationships are preserved. Used by onboarding and
        scheduler-side updates so the space block in the capsule
        ...
        raw, _result = self._store.read_space(space_id)
        if raw is None:
            raw = SpaceGraphSnapshot(
                space_id=space_id,
                composed_at_ms=_now_ms(),
            )
        updated = _replace(
        ...
        write_res = self._store.write_space(updated, writer_id=SPACE_GRAPH_WRITER_ID)
        if not write_res.accepted:  # pragma: no cover - allowlist misconfig
            raise RuntimeError(
                f"projection store rejected space routines write: {write_res.reason!r}"
            )
