...
- Calendar actions expose `calendar.v1` / `calendar_activity_v1` with scheduling, availability, and external-calendar domain tags.
- Tasks actions expose `tasks.v1` / `tasks_activity_v1` with task-management, delegation, and deadline domain tags.
- Reminder user actions expose `reminders.v1` / `reminders_activity_v1` with alerting, notification, and scheduler domain tags. `fire_reminder` remains scheduler-only and carries explicit tool instructions but no prompt template.
- Chores actions expose `chores.v1` / `chores_activity_v1` with recurrence, gamification, and reward-tracking domain tags.
- Shopping actions expose `shopping.v1` / `shopping_activity_v1` with shopping, category, and parent-approval domain tags.
