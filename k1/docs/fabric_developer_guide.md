...
FabricFactory
    |
    +--> WASMProvider (config.module_path = "path/to/module.wasm")
             |
             +--> IWASMRuntime (wasmtime/wasmer implementation)
                      |
                      +--> Sandbox (memory limit, timeout, no I/O)
                               |
                               +--> your_module.wasm (cached after first load)
```
...
- [ ] **6. Implement the server/module**:
  - MCP: Create server.py + handlers.py in `k1/tools/mcp_servers/<name>/`
  - WASM: Write, compile, place .wasm in `k1/tools/wasm_modules/<name>/build/`
- [ ] **7. Define ProviderConfig**: Ensure provider_id matches contract
- [ ] **8. Write integration tests**: Use `FabricFactory.create_for_testing()`
