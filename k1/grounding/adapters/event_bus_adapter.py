...
class EventBusAdapter(IGroundingEventPort):
    """Publish grounding events onto the K1 bus as JSON envelopes."""
    ...
    async def publish(self, topic: str, payload: Mapping[str, Any]) -> None:
        try:
            ...
            logger.warning("Grounding EventBusAdapter: bus envelope import failed", exc_info=True)
            return
        try:
            ...
            logger.warning(
                "Grounding EventBusAdapter: publish failed topic=%s", topic, exc_info=True
            )
