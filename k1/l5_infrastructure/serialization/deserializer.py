"""
FlatBuffers Deserializer - FlatBuffers Binary → Python Objects (Zero-Copy)
...
Architecture Decision Records:
    - ADR-0011: FlatBuffers for All K1 Serialization
      ...
      * 150-200× faster than JSON deserialization (0.05ms vs 7.5ms)
...
class Deserializer:
    ...
    Key Property:
        Traditional JSON: JSON String (64KB) → Parse (7.5ms) → Allocate (3KB) → Copy Fields → Value
        FlatBuffers Zero-Copy: Binary Buffer (64KB) [KEEP IN MEMORY] → Cast Pointer (0.05ms) → Vtable Lookup → Value [Read from buffer at offset, no copy]
...
#   - Compare with JSON baseline (expected 150-200× faster)
