...
## M8 -- Back Capability Execution Kernel Closeout

| Issue | Status | Production files changed | Tests added/updated | Notes |
| --- | --- | --- | --- | --- |
| M8.E1.I1 | green | `k1/concierge/react/back_execution_plan.py`; `k1/concierge/react/loop.py`; `k1/concierge/actors/back.py` | `tests/k1/concierge/react/test_back_execution_plan.py`; `tests/k1/concierge/react/test_back_capability_execution_kernel.py` | Per-intent Back work ledger gates complete submits. |
| M8.E1.I2 | green | `k1/concierge/actors/back.py`; `k1/concierge/react/checkpoint.py` | `tests/k1/concierge/test_react_checkpoint.py`; `tests/k1/concierge/test_back_result_frame.py` | Checkpoints preserve Back execution plan and assistant tool-call history. |
| M8.E1.I3 | green | `k1/concierge/react/loop.py`; `k1/concierge/actors/back.py` | `tests/k1/concierge/react/test_back_capability_execution_kernel.py` | `submit_result(complete)` is rejected while any intent remains uncovered. |
| M8.E1.I4 | green | `k1/concierge/actors/back.py`; `k1/concierge/actors/frames.py` | `tests/k1/concierge/test_back_result_frame.py`; `tests/k1/concierge/test_back_handler_profile_wiring.py` | Back frames carry execution-plan facts without user-facing prose ownership. |
| M8.E2.I1 | green | `k1/concierge/task/parallel_safety.py`; `k1/concierge/tools/parallelism.py`; `k1/concierge/react/loop.py` | `tests/k1/concierge/test_m03_e34_parallel_safety.py`; `tests/k1/concierge/react/test_back_capability_execution_kernel.py` | Concrete tool-call classification is contract-aware and fail-closed. |
| M8.E2.I2 | green | `k1/concierge/tools/implementations.py`; `k1/concierge/ports.py`; `k1/concierge/adapters/fabric_dispatch.py` | `tests/k1/concierge/tools/test_batch_invoke_capabilities.py`; `tests/k1/concierge/ports/test_dispatch_adapter.py` | Batch capability execution uses Fabric ordered parallelism for read-safe groups. |
| M8.E2.I3 | green | `k1/concierge/react/loop.py`; `k1/concierge/task/parallel_safety.py` | `tests/k1/concierge/react/test_back_capability_execution_kernel.py`; `tests/k1/concierge/test_m03_e34_parallel_safety.py` | Batch wrapper remains sequential at ReAct level and handles parallel exceptions as observations. |
| M8.E2.I4 | green | `k1/concierge/tools/implementations.py`; `k1/concierge/task/parallel_safety.py` | `tests/k1/concierge/tools/test_batch_invoke_capabilities.py` | Read-prefix safety cannot override explicit write/side-effect contracts. |
| M8.E3.I1 | green | `k1/fabric/retrieval/retrieval_engine.py`; `k1/concierge/tools/implementations.py` | `tests/k1/fabric/test_retrieval_engine.py`; `tests/k1/concierge/test_tool_fabric_port_wiring.py` | Domain hints are soft, list-valued, and exact-domain indexing is preserved where possible. |
| M8.E3.I2 | green | `k1/fabric/retrieval/soft_ranker.py`; `k1/fabric/types.py`; `k1/contracts/tools/discover_capabilities.yaml` | `tests/k1/fabric/test_retrieval_engine.py`; `tests/k1/fabric/test_discovery_tools_453.py` | Ranking includes generic contract evidence and exposes diagnostics. |
| M8.E3.I3 | green | `k1/concierge/prompt/back_profiles.py`; `k1/concierge/actors/back.py` | `tests/k1/concierge/prompt/test_back_profiles.py`; `tests/k1/concierge/test_back_handler_profile_wiring.py` | Bundled tasks+calendar tasks select both metadata-backed profiles. |
| M8.E3.I4 | green | `k1/concierge/prompt/back_prompt.py`; `k1/concierge/tools/implementations.py` | `tests/k1/concierge/prompt/test_back_prompt_capability_names.py`; `tests/k1/concierge/tools/test_family_member_reference_normalization.py` | Family member names remain user-facing and normalize internally for single and batch invocation. |
| M8.E4.I1 | green | `k1/concierge/react/loop.py`; `k1/concierge/react/back_execution_plan.py` | `tests/k1/concierge/react/test_back_capability_execution_kernel.py` | Deterministic tasks+calendar regression requires both per-intent facts before completion. |
| M8.E4.I2 | green | `k1/concierge/react/back_execution_plan.py`; lifecycle ownership remains in `k1/concierge/fsm/task_bridge.py` | `tests/k1/concierge/react/test_back_execution_plan.py`; `tests/k1/concierge/test_m01_ledger_replay.py`; `tests/k1/concierge/ledger/test_m5_g5_ledger_recovery_at_set_session_state.py` | Back execution plan does not append ledger events or mutate TaskBridge lifecycle state. |
| M8.E4.I3 | green | `k1/concierge/actors/front.py`; `k1/concierge/actors/frames.py` | `tests/k1/concierge/test_front_present_frame.py`; `tests/k1/concierge/test_front_weave_frame.py` | Typed facts drive presentation; raw frame/tool/member slug details do not enter prompt-visible summaries or WEAVE final fallback text. |
| M8.E4.I4 | green | `docs/architecture/kernel_sweep_status.md`; `docs/plans/k1/conceriege_strengthning_plan.md` | Focused M8 closeout runs listed per issue | Closeout matrix added and E5 rows are now closed by BackPool runtime wiring. |
| M8.E5.I1 | green | `k1/concierge/config/kernel.py`; `k1/concierge/config/concierge.py`; `k1/concierge/config/loader.py`; `k1/concierge/config/defaults.yaml`; `k1/concierge/factory.py`; `k1/concierge/session.py` | `tests/k1/concierge/test_m8_e5_backpool_runtime_wiring.py` | Per-session factory/runtime path constructs BackPool, BackTopicRouter, and ReadyQueue; KernelService Tier-2 session reaches the same wiring. |
| M8.E5.I2 | green | `k1/concierge/session.py`; `k1/concierge/actors/back_pool.py`; `k1/concierge/actors/back.py` | `tests/k1/concierge/test_m8_e5_backpool_runtime_wiring.py` | Single `_back_task` gate replaced with pool scheduling; dispatch workers overlap, overflow queues, cancel bypasses slots, and resume/clarification route to Back resume. |
| M8.E5.I3 | green | `k1/concierge/session.py`; `k1/concierge/actors/back_pool.py`; `k1/concierge/actors/ready_queue.py`; `k1/concierge/bus/builders.py` | `tests/k1/concierge/test_m8_e5_backpool_runtime_wiring.py` | Runtime starts/stops the lease watcher, injects lease cancellation tokens, drains overflow, emits BackPool observability, and fails dependents without invoking Back. |
| M8.E5.I4 | green | `tests/k1/concierge/test_m8_e5_backpool_runtime_wiring.py`; `tests/k1/concierge/test_concierge_factory.py`; `docs/architecture/kernel_sweep_status.md`; `docs/plans/k1/conceriege_strengthning_plan.md` | `tests/k1/concierge/test_m8_e5_backpool_runtime_wiring.py`; `tests/k1/concierge/test_concierge_factory.py` | POC BackPool intent promoted into k1 regressions and factory PortBundle expectation refreshed for the existing temporal port. |
