---
adr_number: 0032d
title: Egress Violation Logging & Audit Trail
status: PROPOSED
...
2. Tool POSTs PII to attacker.com ❌ (blocked by iptables)
3. Tool reads /home/user/.ssh/id_rsa ❌ (blocked by chroot)
4. Tool spawns 1000 processes ❌ (blocked by cgroups)

Result: Attacks blocked ✅ BUT:
...
2. Tool POSTs to attacker.com ❌ (LOGGED to K0 ToolReceipt)
3. Tool reads /home/user/.ssh/id_rsa ❌ (LOGGED)
4. Tool spawns 1000 processes ❌ (LOGGED + ALERT sent)
...
Result: Security team alerted, tool blacklisted, compliance audit satisfied ✅
```
...
1. **Violation Detector** → Captures egress violations in real-time
2. **ToolReceipt Writer** → Forwards to K0 audit trail
3. **Alert Manager** → Real-time notifications to security team
...
class ViolationLogger:
    ...
    def __init__(self, k0_receipt_writer, alert_manager):
        ...
        self.violation_counts: Dict[str, int] = defaultdict(int)  # tool_name → count
...
class AlertManager:
    ...
    async def _send_slack_alert(self, violation: EgressViolation):
        ...
        message = {
            "text": f"🚨 *CRITICAL SECURITY VIOLATION* 🚨",
            "blocks": [
                {
                    "type": "header",
                    "text": {
                        "type": "plain_text",
                        "text": "🚨 Critical Egress Violation Detected"
                    }
                },
                {
                    "type": "section",
