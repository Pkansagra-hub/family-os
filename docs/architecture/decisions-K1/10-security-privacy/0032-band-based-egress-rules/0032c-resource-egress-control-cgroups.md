...
- layer4_runtime
affected_modules:
  - k1.l5_infrastructure.egress_control.cgroups_manager
authors:
...
propagation:
  triggers:
    - Changing resource limits or cgroup policies
    - Modifying CPU/memory/IO quotas
  affected_adrs:
  - ADR-0032
  - ADR-0032a
  - ADR-0032b
  ...
  - ADR-0033
  affected_contracts:
  - k1/contracts/flatbuffers/layer3_execution/cgroups_policy.fbs
  affected_tests:
    - tests/k1/l5_infrastructure/test_cgroups_manager.py
related_adrs:
- ADR-0032d
- ADR-0033
- ADR-0033c
...
parent_adr: "ADR-0032"
parent_adr: "ADR-0032"
title: "0032C: Resource Egress Control (cgroups)""0032C: Resource Egress Control (cgroups)"Resource Egress Control (cgroups)
---
...
1. Tool spawns 1000 threads (fork bomb) Ã¢ÂÅ’
2. Tool allocates 16GB RAM (memory exhaustion) Ã¢ÂÅ’
3. Tool runs infinite loop at 100% CPU (DoS) Ã¢ÂÅ’
4. Tool writes 500GB to /tmp/ (disk exhaustion) Ã¢ÂÅ’
5. Tool opens 100,000 file descriptors (FD exhaustion) Ã¢ÂÅ’
...
Execution:
1. Tool spawns 1000 threads Ã¢ÂÅ’ (capped at 100, rest fail)
2. Tool allocates 2GB RAM Ã¢Å“â€¦ (at limit)
3. Tool allocates 3GB RAM Ã¢ÂÅ’ (OOM killer terminates tool)
4. Tool runs at 100% CPU Ã¢Å“â€¦ (capped at 400% = 4 cores)
5. Tool writes 10GB to /tmp/ Ã¢Å“â€¦ (at I/O limit)
6. Tool writes 11GB Ã¢ÂÅ’ (I/O throttled to 0 bytes/sec)

Result: Analysis completes, resource abuse prevented Ã¢Å“â€¦
```
...
1. **CPU Quota** Ã¢â€ â€™ Limit CPU cores per tool
2. **Memory Limit** Ã¢â€ â€™ Hard cap with OOM killer
3. **PID Limit** Ã¢â€ â€™ Prevent fork bombs
4. **I/O Limit** Ã¢â€ â€™ Throttle disk writes
...
resource_egress:
  ...
  green_band:
    cpu:
      quota: 400000        # 4.0 CPUs (400,000 Ã‚Âµs per 100ms period)
...
class ResourcePolicy:
    ...
    tool_pid: int
    band: str
    cpu_quota: int          # Ã‚Âµs per period
    cpu_period: int         # Ã‚Âµs
...
class CgroupsResourceController:
    ...
    def __init__(self, cgroup_root: Path = Path("/sys/fs/cgroup")):
        ...
        self.active_cgroups: Dict[int, Path] = {}  # pid Ã¢â€ â€™ cgroup path
...
**Timeline:** 8 weeks
**Priority:** Ã¢Â­ÂÃ¢Â­ÂÃ¢Â­Â Critical (Security foundation)
