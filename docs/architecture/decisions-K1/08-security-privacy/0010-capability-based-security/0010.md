...
affected_modules:
- k1.l2_orchestration.capability.token_manager
- k1.l3_execution.flow_engine.capability_enforcer
- k1.l4_runtime.session_state.capability_store
- k1.l5_infrastructure.security.capability_registry
- k1.l5_infrastructure.audit.capability_audit_trail
- k0.kernel.capability_validator
affected_tests:
- tests/k1/l2_orchestration/test_capability_token_manager.py
- tests/k1/l3_execution/test_capability_enforcement.py
- tests/k1/l4_runtime/test_capability_store.py
- tests/k1/l5_infrastructure/test_capability_registry.py
- tests/k0/kernel/test_capability_validator.py
- tests/integration/test_capability_based_security.py
authors:
...
propagation:
  ...
  - "Access Control Lists (ACL)** | 3/10 | Simple (list of allowed subjects per resource)<br/>Widely
    understood | â\x9DŒ Ambient authority (ACL check at action time)<br/>â\x9DŒ Confused
    deputy problem (agent can be tricked)<br/>â\x9DŒ No delegation (ACL doesn't transfer
    authority)<br/>â\x9DŒ Revocation slow (must update ACL for each resource) | Ambient
    authority (agent can still call tool if it knows resource ID, no unforgeable token),
    confused deputy problem unsolved |"
  - Adding new module to any layer
  - "Attribute-Based Access Control (ABAC)** | 5/10 | Fine-grained (policies based
    on attributes like cost, time, location)<br/>Flexible (dynamic policies) | â\x9DŒ
    Complex policy evaluation (>10ms latency per check)<br/>â\x9DŒ No delegation (attribute
    checks don't transfer authority)<br/>â\x9DŒ Policy explosion (100+ attributes
    = 1000+ policies) | Too slow (>10ms policy evaluation unacceptable for hot path
    <1ms budget), complex to maintain (policy explosion) |"
  - "Capability-Based Security** âœ… | **9/10** | âœ… **Unforgeable tokens** (HMAC-SHA256
    signed)<br/>âœ… **No ambient authority** (possession of token = only way to access)<br/>âœ…
    **Delegation & attenuation** (pass capability to sub-agent with reduced rights)<br/>âœ…
    **Fine-grained constraints** (max_cost, max_invocations, privacy_band)<br/>âœ…
    **Fast validation** (<1ms HMAC check + constraint validation)<br/>âœ… **Revocation**
    (mark token revoked, no system restart)<br/>âœ… **Audit trail** (every capability
    issuance/usage/revocation logged) | âš ï¸\x8F Token management overhead (must
    issue, store, revoke tokens)<br/>âš ï¸\x8F Delegation complexity (attenuated capabilities
    must preserve constraints) | Selected despite token overhead (overhead <2ms issuance,
    <1ms validation acceptable for security benefits) |"
  - Changing layer dependency rules
  - Modifying system architecture
  - "OAuth2 Scopes** | 6/10 | Token-based (unforgeable tokens like capabilities)<br/>Delegation
    support (refresh tokens) | â\x9DŒ Coarse scopes (scope = \"tool:*\" not \"tool:book_reservation\")<br/>â\x9DŒ
    No attenuation (can't reduce scope on delegation)<br/>â\x9DŒ No constraints (can't
    encode max_cost, max_invocations) | Not expressive enough (can't encode fine-grained
    constraints like \"max_cost_usd: 100.0\" or \"requires_approval: true\") |"
...
- k1/contracts/flatbuffers/layer3_execution/capability_lifecycle.fbs
- k1/contracts/flatbuffers/layer4_runtime/security_policy.fbs
- k1/contracts/flatbuffers/layer5_infrastructure/audit_trail.fbs
- k0/contracts/api/rest/capability_management.yml
- k0/contracts/asyncapi.capability_events.yaml
related_diagrams:
- k1_capability_based_security
- k1_capability_token_lifecycle
- k1_capability_enforcement_flow
- k1_capability_delegation_model
research_citations:
- Capability-Based Computer Systems (Dennis & Van Horn 1966) - Original capability
  model research
- The Confused Deputy Problem (Hardy 1988) - Security vulnerability that capabilities
  solve
- Robust Composition: Towards a Unified Approach to Access Control and Concurrency
    Control (Miller 2006) - Object-capability model
- Capability Myths Demolished (Miller et al. 2003) - Addressing common misconceptions
  about capabilities
- Security Engineering (Anderson 2008) - Least privilege principle and access control
- Access Control: Principles and Practice (Bishop 2002) - Comprehensive access control
    theory
status: ACCEPTED
superseded_by: []
supersedes: []
