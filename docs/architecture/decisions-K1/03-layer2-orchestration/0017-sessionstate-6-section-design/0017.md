...
propagation:
  ...
  affected_tests: []
  triggers:
  - Session state section additions or modifications
  - Eviction policy priority adjustments (beliefs, scoreboard, control, persona, multimodal,
    meta)
  - Serialization format changes for any section
  - SessionState size budget modifications
  - Layer 2 state storage infrastructure changes
related_adrs:
- ADR-0001
- ADR-0001a
- ADR-0001d
...
related_diagrams: []
research_citations:
- Session State Management (Amazon Alexa State Management, 2024)
- Context Persistence (OpenAI Session State, 2024)
- Multi-Agent State Coordination (CSCW Theory, 2020)
status: FROZEN
superseded_by: []
supersedes: []
title: SessionState 12-Section Tiered Design
---
...
### Evolution from 6-Section to 12-Section Design

The original 6-section design evolved into a **12-section tiered architecture** based on:

1. **Edge-First Design**: K1 must work fully offline (no K0 dependency for core operation)
2. **Memory Pressure**: Tighter budgets required HOT/WARM tier separation
3. **40-Turn Retention**: History management required dedicated sections
4. **Single-Writer Pattern**: Concierge-only writes simplified concurrency

### Final Architecture (Implemented)

```
SessionState (96KB total = 48KB HOT + 48KB WARM)
├── HOT CORE (48KB) - Never thrash, always in RAM
│   ├── 1) control (8KB)         - Agent leases, turn lock, flow state [NEVER EVICT]
│   ├── 2) beliefs_active (8KB)  - Facts needed THIS turn
│   ├── 3) scoreboard (6KB)      - Referents, QUD, salience
│   ├── 4) history_active (8KB)  - Last 10 turns (full fidelity)
│   ├── 5) clarifications (4KB)  - Open gaps THIS turn
│   ├── 6) affective_now (4KB)   - Current emotion snapshot
│   ├── 7) narrative_active (4KB)- ONE active thread
│   └── 8) meta (2KB)            - Session ID, band, timestamps
│
└── WARM TIER (48KB) - Evictable to LOCAL COLD
    ├── 9) beliefs_history (12KB)  - Recent facts (eviction priority 2)
    ├── 10) history_recent (20KB)  - Turns 11-40 (lossy compression)
    ├── 11) persona (8KB)          - Traits, style, voice (eviction priority 4)
    └── 12) telemetry (8KB)        - Tokens, cost, counters (eviction priority 1)
```

### Key Changes from Original Design

| Original (6 Sections) | Final (12 Sections) | Rationale |
|----------------------|---------------------|----------|
| beliefs (single) | beliefs_active + beliefs_history | HOT/WARM split for eviction |
| scoreboard | scoreboard (unchanged) | 6KB budget confirmed |
| control | control (NEVER EVICT added) | Critical section protection |
| persona | persona (moved to WARM) | Static data, low priority |
| multimodal | **REMOVED** | Text-only V1, deferred to V2 |
| meta | meta + telemetry | Split: meta=HOT, telemetry=WARM |
| (none) | history_active + history_recent | 40-turn retention policy |
| (none) | clarifications | Open gaps tracking |
| (none) | affective_now | Emotion snapshot |
| (none) | narrative_active | Thread tracking |

### Tiering Model

- **HOT CORE**: Always in RAM, never thrash, sub-100μs reads
- **WARM TIER**: Evictable under pressure, archive to LOCAL COLD
- **LOCAL COLD**: K1 SQLite (offline-safe, no K0 dependency)
- **CLOUD SYNC**: K0 (optional enhancement, async, best-effort)

### Implementation References

- **Source**: `k1/sessionstate/`
- **Diagrams**: `k1/sessionstate/sessionstate_internal.mmd`, `k1/sessionstate/sessionstate.mmd`
- **Contracts**: `k1/contracts/schemas/module/sessionstate.contract.yaml`
