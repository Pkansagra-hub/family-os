...
title: "Model Placement Cascade (NPU→GPU→CPU→Remote)"
...
affected_modules:
  ...
  - k1.l3_execution.inference
  - k1.l4_runtime.device_manager
...
propagation:
  triggers:
    - "Changing placement cascade order (NPU→GPU→CPU→Remote)"
    - "Adding new device types (TPU, VPU)"
    - "Modifying failover strategies"
  affected_adrs:
    - ADR-0024
    - ADR-0026
    - ADR-0027a
  ...
  affected_contracts:
    - k1/contracts/flatbuffers/layer3_execution/model_request.fbs
  affected_tests:
    - tests/k1/l2_orchestration/test_placement_cascade.py

# Research Citations
research_citations:
  - "Edge-Cloud Continuum (Satyanarayanan 2017) - Tiered Inference"
  - "ONNX Runtime Execution Providers - Multi-Device Inference"
- ADR-0031c
- ADR-0050c
- ADR-0053c
- ADR-0057
