...
**PR sequencing within MS-5 (revised 2026-05-06 after PR#1 audit + 3-subagent research):**

| PR | Status | Scope |
| -- | ------ | ----- |
| PR#1 | **SHIPPED** (2026-05-16) | Gateway port + manifest schema additions (`mcp:` block, `delivery.transport: mcp_stdio`) + 3-stage pipeline (no rate-limiter/breaker) + skeleton `MCPProcessManager` + skeleton `InMemoryCredentialVault` + GCal manifest with `signature: PLACEHOLDER_SIGN_BEFORE_REGISTER` + 46 tests + `manifest_bundle_sha = 9f14df579b56fd33a0ab4d0f16f05855a317ef13bfb2bea705d1ab001b55d9a0` |
| **Preflight** | **NEXT** | `CredentialVault` Protocol rename (`put→store`, `get→retrieve`) + add `rotate(adapter_id, key, new_secret)` + `health_check() -> VaultHealth`. Single commit, internal only — no external consumers. **Bundles into PR#2 commit #1.** |
| PR#2 | NEXT | Real `MCPProcessManager` (subprocess spawn via `fastmcp.client.transports.StdioTransport`) + `MCPChild` dataclass + `mcp/initialize` handshake + `tools/list` cache + health pings + `CrashBudget` extracted class + Windows DPAPI `KeyringVault` + audit log + factory + Windows Job-Object-lite (`CREATE_NEW_PROCESS_GROUP` + atexit). New deps: `keyring>=24,<25`. **Slice A: Windows-only sandbox.** |
| PR#2b | After PR#2 | Linux `systemd-run` + macOS `sandbox-exec` sandbox strategies + AES-256-GCM SQLite vault fallback (using `cryptography`'s `AESGCM` + PBKDF2-HMAC-SHA256 from machine fingerprint — NOT `pysqlcipher3`) + `vault_migrate.py` CLI. Parallelizable with PR#3 once PR#2 is in. |
| PR#3 | After PR#2 | **D19 dev anchor + signing pipeline.** Generate dev Ed25519 keypair → check in `tests/fixtures/dev_trust_anchor.{pub,priv}.json` → ship `tooling/contracts/sign_manifest.py` CLI → sign `ifl.google_calendar.events.list.v1.yaml` with dev key → set `ca_bundle.json` to dev key with `ca_id: dev_familyos_root_v1` (distinct id) → ship `bridge/contracts/_meta/ca_bundle.audit.json` stub → 3 new CI gates: `manifest_signature_verifies_against_ca_bundle`, `ca_bundle_audit_consistency`, `ca_bundle_not_placeholder` (mode=`fail`). Flip `trust_unsigned=False` for GCal in gateway tests. **Real production ceremony deferred to "ops PR" (post-MS-5; 1-line key swap + audit append; all gates+tests still pass).** |
| PR#4 | After PR#2 + PR#3 | Real Google Calendar MCP adapter (`bridge/ifl/adapters/google_calendar/{server,oauth,consent}.py`, **fastmcp** Python — not Node) + 4 new K0 IFL tables (`st_ifl_adapter_registry`, `st_ifl_credentials`, `st_ifl_manifests`, `st_ifl_events`) + `k0/pipelines/p_ifl/{ingest,google_calendar_normalizer}.py` (feeds existing P02, **not** new pipeline number) + recorded GCal cassettes via `pytest-recording` + MS-5 exit test `test_ifl_google_calendar_events_list_round_trips_via_gateway`. New deps: `google-auth`, `google-auth-oauthlib`, `google-api-python-client`, `pytest-recording`. Pin `fastmcp>=2.14.0,<3`. Cassette capture is a separate manual step (commit fixtures before PR#4 opens). **Closes MS-5.** |

**Locked decisions (defaults answer to PR#2/3/4 audit questions, captured 2026-05-06):**

1. Vault rename `put→store, get→retrieve` runs as PR#2 commit #1.
2. PR#2 = Slice A — Windows DPAPI keyring + Windows Job-Object-lite; defer Linux/macOS sandboxes + sqlcipher to PR#2b.
3. Stdio framing via `fastmcp.client.transports.StdioTransport` (fall back to hand-rolled JSON-RPC framing only if API doesn't fit; verified before committing).
4. Dev trust anchor uses **distinct** `ca_id: dev_familyos_root_v1`; `ca_bundle_not_placeholder` gate refuses any `dev_*` ca_id when `CI_ALLOW_DEV_TRUST_ANCHOR != 1`.
5. `ca_bundle_not_placeholder` gate mode = **fail** (stays green once dev key replaces PLACEHOLDER).
6. `fastmcp` pinned `>=2.14.0,<3` in PR#4.
7. IFL ingestion path = **bus-based** (`memory.write.v1` over existing MS-3a path); no new K0 HTTP route.
8. K0 P-IFL ingestion feeds existing P02 pipeline; no new pipeline number for v1.
9. GCal cassette capture = manual operator step against a real test account; cassettes committed in a separate fixtures-only PR before PR#4 opens.
10. RateLimiter + CircuitBreaker stages **stay deferred** (manifest schema already reserves the fields); enabled in MS-8 chaos epic 8.7.

**Full PR#2/3/4 implementation detail lives below in Epic 5.2 (PR#2 + PR#2b), Epic 5.3 (PR#2 vault portion), Epic 5.4 (PR#3), Epic 5.5 + 5.6 (PR#4).** Each epic's "Files to create" section is the binding spec. Where the original epic prose conflicted with PR#1 reality (e.g. `connectors/google_calendar/` vs `bridge/ifl/adapters/google_calendar/`, `mcp==1.0.*` vs `fastmcp>=2.14`), the locked-decisions table above wins.
...
| 2026-05-10 → 2026-05-16 | Pair (Prince + Copilot) | **MS-3b, MS-3c, MS-3d, MS-3e, MS-4 CLOSED** + **MS-5 PR#1 SHIPPED.** Detailed closure narratives live in [bridge_system_design.md](bridge_system_design.md) iteration log (entries 2026-05-10 through 2026-05-16) — full deltas, plan-vs-reality reconciliations, gate counts, test counts, and architectural decisions are captured there to avoid duplication. Summary: MS-3b shipped DEGRADED-mode dispatch + LocalOutbox/DrainWorker; MS-3c shipped paired-contract recall surface (deleting legacy hand-written query port per R10); MS-3d shipped 5 SSE topics + real chunked SSE client + K0 endpoint stack; MS-3e shipped obs/feedback channel + K1 detectors; MS-4 shipped json/msgpack/cbor codec layer + `adapter_loc_budget` gate; MS-5 PR#1 shipped `ConnectorGateway` 3-stage pipeline + extended manifest schema (`mcp_stdio` transport, `mcp{}` block, signing fields) + `SkeletonMCPProcessManager` + `InMemoryCredentialVault` + `CABundleAdapterVerifier` (real Ed25519, supports `trust_unsigned=True`) + GCal manifest with PLACEHOLDER signature + 46 new tests (477/477 bridge+tooling pass). Current state at end of 2026-05-16: 9/9 CI gates green, codegen `--check` clean, `manifest_bundle_sha = 9f14df579b56fd33a0ab4d0f16f05855a317ef13bfb2bea705d1ab001b55d9a0`. |
| 2026-05-06 (recommitted) | Pair (Prince + Copilot) | **MS-5 PR sequencing revised after 3-subagent research audit.** The original 5-PR table was opinionated but didn't reconcile against PR#1's shipped reality (skeleton MCP/Vault using sync `put/get` not async `store/retrieve`; `bridge/ifl/adapters/google_calendar/` not `connectors/google_calendar/`; `fastmcp>=2.14` already pinned, plan's `mcp==1.0.*` is stale; AdapterVerifier real Ed25519 already shipped). Three parallel subagents (one per PR) re-derived scope. Revised table now has 6 rows: PR#1 (shipped), Preflight (vault rename `put→store, get→retrieve` + add `rotate`/`health_check`), PR#2 (real MCPProcessManager Slice A — Windows DPAPI keyring + `fastmcp.client.transports.StdioTransport` + Job-Object-lite, defer Linux/macOS sandboxes + sqlcipher to PR#2b), PR#2b (sandboxes + AES-GCM SQLite fallback using `cryptography`'s `AESGCM` not `pysqlcipher3`), PR#3 (D19 dev anchor + signing pipeline + 3 CI gates; real production ceremony deferred to ops PR), PR#4 (real GCal adapter via `fastmcp` Python + K0 IFL ingestion through existing P02 pipeline + recorded cassettes + MS-5 exit test). 10 locked decisions captured in the PR table (cassette capture is manual operator step before PR#4; ingestion is bus-based `memory.write.v1`, not new K0 HTTP route; RateLimiter+CircuitBreaker stay deferred to MS-8 chaos epic 8.7). Next: Preflight + PR#2. |
| 2026-05-17 | Pair (Prince + Copilot) | **MS-5 Preflight + PR#2 SHIPPED.** Preflight: `CredentialVault` Protocol renamed `put→store / get→retrieve`, added `rotate(*, adapter_id, key, new_secret)` raising `CredentialNotFoundError` on missing record + `health_check() -> VaultHealth`. New `VaultHealth(backend, ok, detail)` frozen dataclass exported. `InMemoryCredentialVault` updated to match; only consumer (`tests/bridge/connector/test_gateway_port.py`) constructs the vault, no method calls needed updating. PR#2 (Slice A): extracted `bridge/connector/crash_budget.py` (`CrashBudget` dataclass with sliding-window quarantine + injectable time source, replaces inline deque logic in `SkeletonMCPProcessManager`); shipped `bridge/connector/vault/{__init__,audit_log,keyring_vault,factory}.py` — `VaultAuditLog` JSONL appender (thread-safe, secret-free, hydrated from disk on restart), `KeyringVault` wrapping the system keyring module (Windows DPAPI via `WinVaultKeyring`; index hydrated from audit log so `list_keys` survives restarts; idempotent `delete`; round-trip sentinel `health_check`), `create_vault(backend=…)` factory keyed off `BRIDGE_VAULT_BACKEND` env (memory/keyring; sqlcipher reserved for PR#2b); shipped `bridge/connector/transport/mcp_stdio.py` (`MCPStdioTransport` wrapping `fastmcp.Client` + `StdioTransport` with `open/invoke/ping/close/cached_tools/is_connected`), `bridge/connector/_tool_descriptor_helpers.py` (fastmcp Tool → plain dict → `ToolDescriptor` conversion); shipped `bridge/connector/mcp_child.py` (`MCPChild` dataclass with `mark_ready/unhealthy/stopped/quarantined` lifecycle helpers); shipped `bridge/connector/real_mcp_process_manager.py` (`RealMCPProcessManager` — production replacement for the skeleton: real subprocess via fastmcp StdioTransport, mcp/initialize handshake within `init_timeout_s`, cached tools/list, async invoke with no-lock concurrency + lock-only lifecycle transitions, health() snapshot, ping(), per-child `CrashBudget`, atexit cleanup via `weakref.WeakSet` of live managers, idempotent start/stop). Shipped echo MCP fixture (`tests/fixtures/mcp_servers/echo/server.py` — fastmcp FastMCP server with `echo` + `add` tools, runs as `python -m tests.fixtures.mcp_servers.echo.server`). Added `keyring>=24,<26` + pinned `fastmcp>=2.14.0,<3` in `requirements.txt`. New tests: `test_crash_budget.py` (9), `test_vault_audit_log.py` (10), `test_keyring_vault.py` (16 with fake_keyring shim), `test_vault_factory.py` (8), `test_mcp_stdio_transport.py` (9 spawning real Python subprocess), `test_real_mcp_process_manager.py` (10 incl. quarantine after crash budget exhausted). Total 539 tests pass (was 477; +62 new) + 9/9 gates green. RateLimiter and CircuitBreaker remain deferred to MS-8 epic 8.7 per locked decision #10. **PR#2b deferred to after MS-5 close** (Linux/macOS sandbox profiles + cross-platform AES-GCM SQLite fallback for environments without a system keyring); does not block PR#3 or PR#4. Next: PR#3 (D19 ceremony + sign GCal manifest + flip `trust_unsigned=False`). |
| 2026-05-17 (PR#3) | Pair (Prince + Copilot) | **MS-5 PR#3 SHIPPED.** Generated dev Ed25519 keypair (PyNaCl `SigningKey.generate()`) and committed dev trust anchor: `tests/fixtures/dev_trust_anchor.{priv,pub}.json` (priv-key file is DEV-ONLY — replaced by hardware-backed key during production ceremony). Replaced placeholder `bridge/contracts/_meta/ca_bundle.json` with real bundle (`ca_id: "dev_familyos_root_v1"`, real b64url public key) and added `ca_bundle.audit.json` stub recording method ("PyNaCl SigningKey.generate()"), key storage path, and performed_at timestamp. Shipped `tooling/contracts/sign_manifest.py` CLI: `_canonical_signing_payload` mirrors verifier exactly (`json.dumps(sort_keys=True, separators=(",",":"))` excluding `signing` block); writes `signing.signature` (b64url, no padding) + `signing.payload_sha256` (hex); preserves manifest YAML order via `sort_keys=False` on write. Re-signed `bridge/contracts/manifests/ifl.google_calendar.events.list.v1.yaml` with real signature; round-trips cleanly through production `CABundleAdapterVerifier()` (no `trust_unsigned`). Three new CI gates added (now 12/12 total): (1) `ca_bundle_not_placeholder` — refuses placeholder pubkeys and refuses `dev_*` ca_ids unless `CI_ALLOW_DEV_TRUST_ANCHOR=1` is set, (2) `manifest_signatures_valid` — verifies every IFL adapter manifest (`delivery.transport == "mcp_stdio"`) against production verifier; bus contract manifests (HTTP transport) are explicitly out of scope (covered by `schema_checksum_stable`), (3) `dev_trust_anchor_audit_present` — for `dev_*` ca_ids requires `ca_bundle.audit.json` with matching ca_id + non-blank `performed_at`/`method`/`key_storage`. Updated `_make_minimal_repo` to copy real ca_bundle + audit fixtures so the orchestrator-green test stays green; `_run` now sets `CI_ALLOW_DEV_TRUST_ANCHOR=1` for fixture runs. Codegen regenerated against new manifest_bundle_sha (`1a058d4e8e215f9f6f820aed8ad89afe17b30665d07d871be0fe21905bd84161`). Test deltas: `test_meta_schema.py` placeholder assertion inverted; `test_adapter_verifier.py::test_verifier_rejects_placeholder_public_key_when_strict` rewritten to inject placeholder bundle into `tmp_path` (production bundle is no longer placeholder). New tests: `tests/tooling/contracts/test_sign_manifest.py` (12 — CLI + canonical payload + dev-anchor file invariants), `tests/tooling/ci/gates/test_pr3_signing_gates.py` (13 — fake-repo fixtures for all three gates including bus-manifest-skipped + unsigned-IFL-fails). Total **564 tests pass** (was 539; +25 new) + 12/12 gates green. Production-ceremony PR remains deferred (hardware HSM, valid_until rotation, multi-CA bundle); does not block PR#4. Next: PR#4 (real GCal adapter via fastmcp Python + K0 IFL ingestion + MS-5 exit test). |
| 2026-05-17 (PR#4 — MS-5 CLOSED) | Pair (Prince + Copilot) | **MS-5 PR#4 SHIPPED — MS-5 CLOSED.** Real Google Calendar IFL adapter delivered end-to-end. Shipped `bridge/ifl/adapters/google_calendar/{__init__,server,oauth,consent}.py`: (1) `server.py` — fastmcp `FastMCP("family-os-google-calendar")` exposing the `events_list` tool; pure-Python `events_list_handler` callable for unit tests; production path lazy-imports `httpx` and exchanges OAuth refresh-token + calls Google Calendar v3 API with response-shape normalization (`startTime/dateTime/timeZone` → snake_case schema fields, drops unknown fields); fixture path replays `BRIDGE_GCAL_FIXTURE_PATH` JSON file for hermetic tests (no network required). (2) `oauth.py` — minimal RFC 6749 §6 refresh-token flow against `https://oauth2.googleapis.com/token` with injectable `http_post` for testing; `OAuthError` carries no secrets; `AccessToken` frozen dataclass. (3) `consent.py` — append-only JSONL `ConsentLedger` with `record_grant`/`grants_for_account`/`has_active_grant`; revocation modeled as new entry (`decision="revoked"`) to keep audit trail immutable; thread-safe via `threading.Lock`. K0 side shipped `k0/pipelines/p_ifl/{__init__,google_calendar_normalizer,ingest}.py`: per locked decision #8 there is **no new pipeline number** — IFL ingest drives the existing P02 pipeline. `google_calendar_normalizer.normalize_response` converts a GCal events list → list of v2.2 `MemoryAtom` payloads (`source_type=device_observed`, `temporal_orientation=FUTURE_COMMITMENT`, `elaboration_depth=MENTION`); provenance encoded into `session_id` (`f"ifl_gcal_{account_id}_{event_id}_{outer_session}"`) so the strict v2.2 schema (`extra: forbid`) accepts the payload without bespoke field. `ingest.ingest_atoms` validates each payload against the generated `MemoryWriteV1` Pydantic model and forwards to `p02_write_ingest.ingest`; returns `IFLIngestResult` per atom. **MS-5 EXIT criterion test** (`test_ifl_google_calendar_events_list_round_trips_via_gateway`): drives the full path — `RealMCPProcessManager` spawns the real fastmcp stdio subprocess (running `bridge.ifl.adapters.google_calendar.server` in fixture mode) → `ConnectorGateway` with **production** `CABundleAdapterVerifier()` (no `trust_unsigned`) → manifest re-signed at test time with the dev anchor (because we mutate `mcp.server_command` to `sys.executable` + inject `mcp.env.BRIDGE_GCAL_FIXTURE_PATH`) → `gw.invoke("google_calendar_acc_1", "events_list", …)` → response normalized → `ifl_ingest.ingest_atoms` → P02 acks. Two events round-trip cleanly. New tests: `tests/bridge/ifl/test_google_calendar_round_trip.py` (9 — handler unit, normalizer unit, ingest unit, full MS-5 EXIT round-trip), `tests/bridge/ifl/test_google_calendar_oauth_consent.py` (9 — OAuth helper with fake POST, ConsentLedger CRUD + revocation). Total **582 tests pass** in scoped bridge+tooling suite (was 564; +18 new) + 12/12 gates green. **PR#2b deferred** (Linux/macOS sandbox profiles + cross-platform AES-GCM SQLite vault fallback) — does not block production deployment of the existing Windows/keyring path. **MS-5 CLOSED.** Next milestone: MS-6 (per the master roadmap — concierge surface or operator portal). Pre-existing P03 KG consolidator failures (26 tests in `tests/k0/pipelines/p03/`) are unrelated to MS-5 and pre-date PR#3 (verified by re-running on a clean stash); to be addressed under their owning epic. |
| 2026-05-18 (MS-7 PR#1) | Pair (Prince + Copilot) | **MS-7 Epic 7.1 SHIPPED — live-system test harness.** Adapted to the user's real deployment shape: K0 already runs in **Docker** (kernel `localhost:8080`, postgres `5432`, et al.); the harness only spawns K1 locally. Shipped `tests/integration/harness/`: (1) `port_allocator.py` — ephemeral OS-assigned ports via `socket.bind(("127.0.0.1", 0))`. (2) `leak_detector.py` — `LeakDetector` tracks PIDs (psutil-based liveness probe with signal-0 fallback) / TCP ports / tempdirs; `assert_clean()` raises with a formatted `LeakReport`. (3) `process_supervisor.py` — `ProcessSupervisor.spawn(...)` returns a `ProcessHandle`; `terminate()` sends `CTRL_BREAK_EVENT` on Windows (or SIGTERM elsewhere), waits the configured grace (default 10s), then `Process.kill()`. Spawn uses `CREATE_NEW_PROCESS_GROUP` on Windows for graceful signalling. Stdout/stderr can be redirected to a per-child log file. (4) `k0_handle.py` — **attach mode** is the default (and only supported) mode in this PR. `K0Handle.attach()` polls `/healthz` until `status=="ok"` (raises `K0NotReachableError` after `wait_seconds`); exposes `healthz`/`readyz`/`metrics`. `signal()` raises `NotSupportedInAttachMode` (Docker owns lifecycle). `spawn_k0=True` is reserved for a future testcontainers PR. (5) `k1_handle.py` — `spawn_k1(...)` runs `python -m k1.kernel.runner --log-level WARNING` as a real subprocess with env `K0_BASE_URL`/`K1_PERSON_ID`/`K1_DEVICE_ID`/`K1_FAMILY_ID`/`K1_TEST_API_PORT`. Each handle additionally exposes a lazily-built in-process `BridgeRuntime.from_registry(role=K1, transport=HttpTransport(TransportConfig(base_url=k0_base_url)))` so tests can publish `memory.write.v1` / drive `query.recall` against the live K0 with the K1's identity *while* the subprocess remains real (lifecycle isolation, SIGTERM testing). A subprocess-driven test API is deferred to a later PR (the harness contract is stable across that change). (6) `family_layout.py` — `PersonSpec` / `DeviceSpec` / `FamilyLayout` + `default_father_mother_kid_layout()` (3 people, 3 devices) and `single_father_layout()`. (7) `live_system.py` — `LiveSystem` async context manager: attaches K0, allocates ephemeral ports, spawns one K1 per (person, device), tracks all in `LeakDetector`. `__aexit__` calls `supervisor.shutdown_all()` → removes tempdirs → `assert_clean()`. (8) `conftest.py` — `requires_live_k0` marker auto-skips when K0 is unreachable; `live_k0` and `live_system` fixtures. (9) `test_harness_self.py` — **15 self-tests, all green** against the running Docker K0: 9 pure-unit tests (port allocator, leak detector for PIDs/ports/tempdirs, family layouts), 3 K0Handle attach tests (healthz, metrics, bogus-URL failure), 1 supervisor SIGTERM round-trip on a `time.sleep(60)` child, **2 full LiveSystem smoke tests** that spawn 1 and 3 real K1 subprocesses against the live Dockered K0 and verify clean teardown via `LeakDetector.assert_clean()`. (10) `README.md` — usage, env vars, deferred items. Added `psutil>=5.9,<6` requirement (testcontainers intentionally NOT added — attach mode is the default and Docker is provided by the operator). Deferred: `spawn_k0=True` mode (testcontainers), per-test PG schema isolation (`AttachExistingK0Backend.create_schema`), subprocess-driven K1 test API, SQLite fast-loop backend. Next: Epic 7.2 (partition isolation — 14 tests) and 7.3 (5 user journeys). |
