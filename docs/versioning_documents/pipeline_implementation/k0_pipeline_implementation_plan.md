...
- `k0/storage/migrations/003_pipeline_ledger.sql` (NEW)
- `k0/storage/migrations/004_dlq.sql` (NEW)

**DDL Required:**

**Migration 003: Pipeline Ledger**

```sql
-- Add monotonic offset to WAL
ALTER TABLE st_wal ADD COLUMN wal_pos INTEGER PRIMARY KEY AUTOINCREMENT;

-- Update outbox to reference wal_pos
ALTER TABLE st_outbox ADD COLUMN wal_pos INTEGER NOT NULL REFERENCES st_wal(wal_pos);
...
**Migration 004: Dead Letter Queue**
...
CREATE INDEX idx_dlq_fingerprint ON st_dlq(error_fingerprint);
CREATE INDEX idx_dlq_topic ON st_dlq(topic);
```

**Acceptance Criteria:**

- [ ] Migration 003 runs successfully (idempotent)
- [ ] Migration 004 runs successfully (idempotent)
- [ ] Schema verified: `sqlite3 k0_runtime.sqlite3 ".schema st_pipeline_processed"`
- [ ] All 4 tables exist: `st_pipeline_processed`, `st_pipeline_status`, `st_pipeline_watermarks`, `st_dlq`
...
- `k0/storage/migrations/003_pipeline_ledger.sql`
- `k0/storage/migrations/004_dlq.sql`
- Test: `tests/k0/storage/test_migrations_pipeline.py`
...
**Duration:** 4 hours
**Status:** 🔴 Not Started
**Dependencies:** M1 (requires BusDispatcher v2, PipelineProtocol, DDL tables)
**Blocking:** M3, M4, M5
