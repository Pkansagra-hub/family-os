...
sqlite3 d:\familyos\k0_runtime.sqlite3
...
# Verify provisioning
SELECT * FROM devices WHERE device_id = 'device-mobile-001';
```
...
class K0DeviceClient:
    ...
    def submit_command(self, topic, payload, schema_uri):
        ...
        Args:
            topic: Command topic (e.g., "commands.delta.memory.write")
        ...
        envelope = {
            ...
            "idem_key": hashlib.sha256(
                f"{self.device_id}:{topic}:{json.dumps(payload)}".encode()
            ).hexdigest()
        }
        ...
        # 5. Add signature to envelope
        envelope['sig'] = sig_hex

        # 6. Submit to kernel
        response = requests.post(
            f"{self.kernel_url}/k0/command.submit",
            json=envelope,
            timeout=self.kernel_url.get('timeout_ms', 5000) / 1000
        )
...
# Usage example
config = {
    'kernel': {'host': 'localhost', 'port': 8080, 'timeout_ms': 5000},
    'device': {
        'tenant_id': 'tenant-001',
        'space_id': 'space-home',
        'device_id': 'device-mobile-001'
...
sqlite3 d:\familyos\k0_runtime.sqlite3 "SELECT device_id, roles FROM devices WHERE device_id = 'device-mobile-001';"

# 2. Verify policy manifest
cat d:\familyos\k0\deploy\generated\manifests\allow_all.json | grep -A 5 '"roles"'

# 3. Update device roles if needed
sqlite3 d:\familyos\k0_runtime.sqlite3 "UPDATE devices SET roles = 'device,admin' WHERE device_id = 'device-mobile-001';"

# 4. Restart kernel to reload policy cache
...
sqlite3 d:\familyos\k0_runtime.sqlite3 "SELECT schema_uri, version, status FROM schemas;"

# 2. Register missing schema
sqlite3 d:\familyos\k0_runtime.sqlite3 << 'EOF'
INSERT INTO schemas (schema_uri, version, schema_json, status, created_at)
VALUES (
  'schema://memory.delta',
  '1.0',
...
# 3. Verify activation
sqlite3 d:\familyos\k0_runtime.sqlite3 "SELECT * FROM schemas WHERE schema_uri = 'schema://memory.delta';"
```
...
- Second submission: 409 Conflict with cached receipt

**Verification**:
```bash
# Check idempotency ledger
sqlite3 d:\familyos\k0_runtime.sqlite3 "SELECT idem_key, receipt_id, created_at FROM idempotency_ledger ORDER BY created_at DESC LIMIT 5;"

# If duplicates expected, this is normal:
# idem_key|receipt_id|created_at
# abc123...|receipt-1|2025-10-31 14:01:32
# abc123...|receipt-1|2025-10-31 14:01:32  <- Cache hit returns same receipt
```
