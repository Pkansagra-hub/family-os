...
| Issue | Title | Status | Deliverable | Details |
|-------|-------|--------|-------------|---------|
| 6.2.1 | Test ContractValidator | DONE | test_contract_validator.py | Test all 12 validation rules: (1) name pattern, (2) semver, (3) domain non-empty, (4) description non-empty, (5) required_inputs complete, (6) output schema valid, (7) provider_type enum, (8) safety_band enum, (9) availability enum, (10) agent tools_granted valid, (11) prompt variables satisfiable, (12) workflow DAG acyclic. Test valid contracts pass. Test each invalid case raises ContractValidationError with clear message. 839 lines, all 12 rules + Phase 1 schema + validate_or_raise + multi-error accumulation. |
| 6.2.2 | Test ContractParsers (all 4 types) | DONE | test_contract_parsers.py | Test YAML loading for tool, agent, prompt, workflow contracts. Test parse -> validate -> dataclass roundtrip. Test malformed YAML handling. Test missing required fields. Test auto-detection of contract type from YAML content. 368 lines, parse_contract() facade + parse_contract_body() + malformed YAML + skip_validation + shared validator caching. |
| 6.2.3 | Test CapabilityRegistry | DONE | test_registry.py | Test register/unregister, lookup (O(1)), list_by_domain, list_by_type, update_availability, update_metrics, reload(), health(). Test thread-safety with concurrent register/lookup. Test event emission on register/unregister. Test with 100+ contracts. 636 lines, version conflict resolution + domain/type/provider indexes + metadata cache + 20-thread concurrency. |
| 6.2.4 | Test ModuleLoader | DONE | test_module_loader.py | Test startup scan of contracts directory. Test hot-reload: file create triggers register, file modify triggers unregister + register, file delete triggers unregister. Test invalid file: logs error, keeps old contract. Test scan with mix of valid and invalid YAMLs. 46/46 tests passing. |
| 6.2.5 | Test ProviderResolution pipeline | DONE | test_provider_resolution.py | Test full pipeline: registry lookup -> provider matching -> policy -> selection -> instantiation. Test capability_not_found error. Test access_denied for safety band violation. Test deterministic selection (FAB-10). Test multi-provider selection. 36/36 tests passing. |
| 6.2.6 | Test PolicyEngine (all 4 dimensions) | DONE | test_policy_engine.py | Test SecurityContext: GREEN user + GREEN cap = PASS, GREEN user + AMBER cap = REJECT. Test AffectiveRouting: high sadness boosts gentle providers. Test CognitiveLoadRouting: high load prefers faster. Test QoSIntegration: tight budget prefers cheaper. Test composite score calculation. Test ToolScope enforcement. 56/56 tests passing. |
| 6.2.7 | Test RetrievalEngine | DONE | test_retrieval_engine.py | Test full pipeline: embed -> hard filter -> soft rank -> Top-K. Test with 50 contracts: verify top results semantically relevant. Test hard filter: safety band exclusion, OFFLINE exclusion, input satisfiability. Test soft ranker weights (0.4/0.3/0.15/0.15). Test DEGRADED penalty (0.7x). Test top_k parameter (default 10, max 25). 53/53 tests passing. |
| 6.2.8 | Test ContextBuilder | DONE | test_context_builder.py | Test 6-step flow: (1) read required_context, (2) fetch from SessionState, (3) inject params, (4) resolve prompt, (5) apply token budget, (6) package. Test optional section missing (skip gracefully). Test required section missing (log warning, partial context). Test token budget compression strategies (5 levels). Test 128K ceiling enforcement (FAB-08). 30/30 tests passing. |
| 6.2.9 | Test CircuitBreaker | DONE | test_circuit_breaker.py | Test state transitions: CLOSED -> OPEN (after threshold failures) -> HALF_OPEN (after recovery period) -> CLOSED (on success) or OPEN (on failure). Test sliding window failure counting. Test per-provider configs. Test retry strategy (max 2 retries). 25/25 tests passing. |
| 6.2.10 | Test Core Types | DONE | test_types.py | Test all dataclasses: CapabilityRequest, CapabilityResult, CapabilityContract, AgentContract, PromptContract, WorkflowContract, RetrievalResult, ExecutionContext, ProviderConfig. Test to_dict(), from_dict() roundtrip. Test factory methods (CapabilityResult.success(), .failure(), .timeout()). Test all enums. 84/84 tests passing. |
| 6.2.11 | Test OutputValidationPipeline | DONE | test_output_validation.py | Test 3-tier pipeline: (1) StructuralValidator rejects malformed results, (2) SchemaValidator rejects schema-violating data, (3) SemanticValidator flags low-confidence outputs. Test ValidationFallback: structural fail -> rejection, schema fail -> coercion attempt, semantic fail -> warning annotation. Test pipeline short-circuit on hard failure. Test configurable skip_semantic for tool results. 58/58 tests passing. |
| 6.2.12 | Test HealthChecker | DONE | test_health_checker.py | Test periodic health checks: healthy provider returns ONLINE, unhealthy returns DEGRADED/OFFLINE. Test consecutive failure threshold for OFFLINE marking. Test recovery progression: OFFLINE -> DEGRADED -> ONLINE. Test health-checker + circuit-breaker bidirectional integration. Test AvailabilityTracker state transitions and history. 70/70 tests passing. |
| 6.2.13 | Test CapabilityVersioning | DONE | test_capability_versioning.py | Test CapabilityVersion parsing, comparison, compatibility. Test registry version conflict resolution: duplicate rejection, compatible upgrade, breaking multi-version, regression rejection. Test version-aware lookup: exact, latest compatible, latest. Test version conflict event emission. 64/64 tests passing. |
...
| Issue | Title | Status | Deliverable | Details |
|-------|-------|--------|-------------|---------|
| 6.3.1 | Test LOW tier execution flow | DONE | test_low_tier_flow.py | 33 tests. Full LOW tier: register tool contract -> fabric.execute(tool request) -> verify CapabilityResult success -> verify events emitted (invoked + completed + learning_signal) -> verify trace_id propagation. Tests MCP, WASM, Bridge provider types. Cross-provider parametrized tests, sequential executions, unregistered capability graceful failure. |
| 6.3.2 | Test MEDIUM tier execution flow | DONE | test_medium_tier_flow.py | 19 tests. PARALLEL + SEQUENTIAL BatchStrategy. Mixed providers (MCP + WASM + BRIDGE) in single batch. Partial failure handling (one invalid + one valid). Backpressure: batch > max_batch_size raises BatchSizeExceededError. Event verification for all batch items. |
| 6.3.3 | Test HIGH tier retrieval + execution flow | DONE | test_high_tier_flow.py | 22 tests. Bulk registration (50-100 contracts), discover_capabilities() with domain filtering, top_k, scored results sorted descending. Discover-then-execute full pipeline for MCP/WASM/BRIDGE. Registration event verification. Lookup after register. Safety band filtering. |
| 6.3.4 | Test agent spawn and execution | DONE | test_agent_execution.py (32 tests) | Register agent contract. Execute via fabric.execute() with agent capability name. Verify: agent lifecycle (PENDING -> WARMING -> ACTIVE -> result -> IDLE/TERMINATED), tool scoping enforced, context built from SessionState, delta emission on fact discovery, CapabilityResult returned. |
| 6.3.5 | Test safety band enforcement end-to-end | DONE | test_safety_band_e2e.py (24 tests) | Register capabilities with different safety_band_min (GREEN, AMBER, RED). Execute with GREEN user: verify GREEN-only caps succeed, AMBER+ rejected with access_denied. Test with AMBER user: GREEN + AMBER succeed, RED rejected. Verify FAB-06. |
| 6.3.6 | Test circuit breaker integration | DONE | test_circuit_breaker_integration.py (18 tests) | Register capability with a flaky provider (3 failures, then success). Verify: first 3 attempts fail, circuit opens, requests rejected without calling provider, after recovery period half-open, success closes circuit. Verify FAB-04 (30s timeout). |
| 6.3.7 | Test context builder with real SessionState | DONE | test_context_with_sessionstate.py (31 tests) | ContextBuilder 6-step pipeline with real TestSessionStateReaderAdapter. Required/optional sections, missing sections, graceful degradation. Token budget 128K ceiling (FAB-08). Compression levels L1/L2. Frozen ExecutionContext. Programmatic contracts with context via fabric.execute(). |
| 6.3.8 | Test sub-agent tool scoping | DONE | test_tool_scoping.py (22 tests) | ToolScope validate()/is_allowed(), AccessDeniedError with check/detail, frozenset immutability, ToolScopeError for empty grants. ToolScope from AgentContract and YAML fixtures. Fabric integration: granted tool executes, ungranted tool denied (FAB-07). |
| 6.3.9 | Test statelessness (FAB-03) | DONE | test_statelessness.py (16 tests) | Sequential capabilities independent, no residual request state on facade, 100 sequential + 10 concurrent executions with unique request_ids, unique cognitive_trace_ids per event (FAB-09), failure isolation (failed then success, interleaved). |
| 6.3.10 | Test deterministic selection (FAB-10) | DONE | test_deterministic_selection.py (24 tests) | ProviderSelector deterministic ordering: highest score wins, tie-break by lower avg_latency_ms then alphabetical provider_id. 100 selections with same inputs produce identical results. Through fabric.execute(): same capability always same provider. select_top_n() ranked deterministic. Edge cases: single candidate, all rejected, no candidates, filtered candidates. Resolved metadata carries correct config/contract/policy. |
| 6.3.11 | Test BridgeConnectionAdapter integration | DONE | test_bridge_connection_adapter.py (39 tests) | LOCAL COLD mode (no client): is_available()==False, all ops return k0_offline BridgeCommandResult, no crash. Bridge available: send_command/query/route_ifl route through InMemoryBridgeClient, probe_connection on init. Client errors: bridge_error result, marks disconnected, recovery via reconnect. Auto-reconnect: throttle, max attempts, state reset. IFLRoute.parse() for home/device/nested addresses, invalid raises ValueError. Health transitions: offline->full, full->offline on error, offline->full via reconnect. BridgeCommandResult.ok()/.fail() factory methods. |
| 6.3.12 | Test execute_batch() semantics | DONE | test_execute_batch.py (31 tests) | PARALLEL strategy: 10 capabilities concurrent, results in input order, single-item, fixture contracts. SEQUENTIAL strategy: ordered execution, results in input order. Partial failure: mix valid+invalid, sequential failure no block, result ordering preserved. Backpressure: exceed max raises BatchSizeExceededError, at max succeeds, empty returns empty, custom max_batch_size. DAG: no deps, linear chain, parallel+join, cycle detection (dag_cycle_detected), self-reference cycle, wave ordering, unknown dep ignored. Cross-strategy invariants: result count, all CapabilityResult, empty batch. |
...
| Issue | Title | Status | Deliverable | Details |
|-------|-------|--------|-------------|---------|
| 6.4.1 | Test module.contract.yaml compliance | DONE | test_module_contract.py | 69 tests (68 passed, 1 xfail). Verifies 24 exact-name exports + 2 renamed (SemanticIndex->EmbeddingIndex, ProviderResolver->Resolver) + 1 xfail (PolicyGuard). 5 port Protocols with method checks. Factory methods. Module dependencies. |
| 6.4.2 | Test wiring.contract.yaml compliance | DONE | test_wiring_contract.py | 111 tests (104 passed, 7 xfail). 31 required files + 9 renamed files + 11 init files. 6 port Protocols with method checks. 8 test adapters + 5 production adapters. Factory wiring (12 checks). 5 capability provisions. Forbidden patterns. Wiring imports + FAB-01 writer prohibition. |
...
| Issue | Title | Status | Deliverable | Details |
|-------|-------|--------|-------------|---------|
| 7.3.1 | Add cognitive_trace_id to all operations | DONE | All subsystems | Every operation logs trace_id. CapabilityRequest.trace_id propagated through: resolve, policy, context_build, execute, result_return. Events carry trace_id (FAB-09). Same pattern as SessionState 5.3.1. |
| 7.3.2 | Implement structured logging | DONE | k1/fabric/logging.py | JSON structured logs per phase: resolve, policy_check, context_build, execute, result_return. Fields: timestamp, level, component (fabric.{phase}), trace_id, request_id, capability_name, provider_id, duration_ms, success. |
...
| Issue | Title | Status | Deliverable | Details |
|-------|-------|--------|-------------|---------|
| 7.4.1 | Define alert rules | DONE | k1/fabric/alerts.yaml | Alerts: `FabricHighLatency` (P95 > 200ms sustained 5min), `FabricCircuitBreakerOpen` (any CB open > 2min), `FabricRetrievalSlow` (P95 > 100ms sustained 5min), `FabricAgentPoolExhausted` (pool=0 for any contract), `FabricRegistryEmpty` (size=0 for any type). |
| 7.4.2 | Document runbook for alerts | DONE | docs/runbooks/fabric.md | Per alert: Cause, impact, investigation steps, remediation. |
...
| Issue | Title | Status | Deliverable | Details |
|-------|-------|--------|-------------|---------|
| 8.1.1 | Test concurrent executions | DONE | test_load_testing.py | Test 10, 50, 100 concurrent fabric.execute() calls. Measure: throughput (ops/sec), latency (P50, P95, P99), error rate. Target: 100 concurrent, P95 <200ms overhead. |
| 8.1.2 | Test registry at scale | DONE | test_load_testing.py | Load 10K, 50K, 100K contracts. Measure: register throughput, lookup latency, retrieval latency. Verify lookup <1ms at 100K. Verify retrieval <50ms at 100K. |
| 8.1.3 | Test retrieval throughput | DONE | test_load_testing.py | With 10K contracts. 1000 concurrent discover_capabilities() calls. Measure QPS, P95 latency. Target: 1000 QPS, P95 <50ms. |
| 8.1.4 | Test agent pool under load | DONE | test_load_testing.py | Spawn 20 agents concurrently. Then 50. Measure: spawn latency, pool reuse rate, memory consumption. Verify pool reuse for same contract type within 60s TTL. |
