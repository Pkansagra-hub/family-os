...
## M8 -- Back Capability Execution Kernel

**Purpose:** Make Back a deterministic capability executor instead of an LLM-only scheduler. Back must cover every dispatched intent, discover the right contract surface, execute safe read batches in parallel when the contracts allow it, preserve write order and HIL gates, and return typed `BackResultFrame` facts/artifacts that Front can present naturally.

**Production bug this milestone fixes:** A dispatch such as "show all home tasks and all calendar events" currently can discover candidates, execute only the calendar read, rediscover again, then terminate with `REACT_LOOP_DEGENERATE`. The root causes are not one prompt line: broad domain hints can hard-filter away the tasks contract, Back has no deterministic per-intent coverage ledger, live `invoke_capability` execution is sequential by policy, and `batch_invoke_capabilities` saves tool budget but still loops through dispatches sequentially internally.

**Second production migration bug this milestone fixes:** The real k1 Concierge runtime still uses one Back LLM slot. The POC/V3 BackPool design and actor modules exist, but the live path from `KernelService._create_session_tier2` to `ConciergeRuntime._mailbox_consumer` never constructs `BackPool`, never instantiates `BackTopicRouter`, never calls `BackPool.acquire_worker`, never starts the lease watcher, and never drains ReadyQueue/overflow. M8 therefore must fix both levels: deterministic work coverage inside each Back task and multiple Back workers at the runtime boundary.

**Prerequisite status in current code:** M2 typed actor handoffs are already present. Treat `BackResultFrame`, `HILResolutionFrame`, and `WeavePresentationFrame` as active contracts, not future placeholders.

**Verified current anchors (2026-05-19, before M8 implementation):**

- Front dispatch schema bundles independent intents in one `dispatch_task`: `k1/concierge/tools/schemas_front.py` L471, L482, L491, L526, L556.
- Front dispatch builder call is in `k1/concierge/actors/front.py` L1515 and tier budget injection is at L1519.
- Task classifier already distinguishes bundled and chained work: `k1/concierge/task/classifier.py` L57, L90, L115, L133.
- Back imports typed frames at `k1/concierge/actors/back.py` L45, selects execution profiles at L453, calls `react_loop` at L1070 and L1433, and emits typed results through `_emit_back_result` at L731 and `BackResultFrame.from_dict` at L776.
- `BackResultFrame` fields are defined in `k1/concierge/actors/frames.py` L124-L154, with legacy fallback parsing at L158-L190. `HILResolutionFrame` starts at L311 and `WeavePresentationFrame` starts at L397.
- Front consumes Back frames in PRESENT mode at `k1/concierge/actors/front.py` L410, WEAVE mode at L432, and strips leaked worker frame text at L288 and L1579.
- Live ReAct parallel classification imports `classify_tool_batch` from `k1.concierge.task.parallel_safety` in `k1/concierge/react/loop.py` L43 and calls it at L1663.
- ReAct already tracks Back authority tools in `k1/concierge/react/loop.py` L326, discovery payloads at L1758-L1763, authority attempts at L1765-L1766, and capability spin nudges at L1842-L1866.
- ReAct has collection read/mutation helpers in `k1/concierge/react/loop.py` L510, L557, L637, and L702, but those helpers are not wired into the main loop path.
- Submit-result processing currently returns immediately from `k1/concierge/react/loop.py` L1628-L1654 with no per-intent coverage gate.
- Live parallel policy in `k1/concierge/task/parallel_safety.py` marks `discover_capabilities` read-safe at L40, but forces `invoke_capability`, `spawn_via_fabric`, `execute_workflow`, `dispatch_task`, `submit_result`, and `promote_belief` sequential at L55-L61; `classify_tool_batch` starts at L84.
- Exported but non-live parallel helper `k1/concierge/tools/parallelism.py` has a conflicting action group that includes `invoke_capability`, `spawn_via_fabric`, and `execute_workflow` at L42-L44, with `ALWAYS_SEQUENTIAL` only including `dispatch_task` and `submit_result` at L50.
- Back LOW/simple tool surface already exposes `batch_invoke_capabilities`: `k1/concierge/tools/schemas_back.py` L37, L303, L309, L320, L330-L336.
- Tool implementations enter capability discovery/invoke/batch at `k1/concierge/tools/implementations.py` L1401, L1558, and L1733. Batch currently loops `for inv in invocations` at L1773 and calls `ctx.dispatch.dispatch_direct` at L1841.
- `submit_result` implementation starts at `k1/concierge/tools/implementations.py` L2048 and forwards complete payload fields at L2071-L2088.
- `FabricDispatchAdapter.execute_batch` currently returns `[await self.dispatch_direct(r) for r in requests]` in `k1/concierge/adapters/fabric_dispatch.py` L363-L366. The same adapter exposes `dispatch_direct` at L78 and `discover_capabilities` at L369.
- Fabric itself already supports true batch strategy selection: `k1/fabric/fabric.py` `BatchStrategy` at L89, `execute_batch` at L727, parallel branch at L766, sequential branch at L768, `_execute_batch_parallel` at L1229, and `_execute_batch_sequential` at L1269.
- Retrieval hard-filters by exact domain tags in `k1/fabric/retrieval/retrieval_engine.py` L166, L191, L224, L246, and L273-L283; empty scoped candidates return before global fallback at L297.
- Registry domain lookup is exact: `k1/fabric/core/registry.py` L667-L680, with domain indexes populated at L1251-L1253.
- Family contract domains come from manifest translation in `k1/fabric/manifest_translator.py` L167, L182-L184, L196, and L221.
- Tasks contract profile/tags are `tasks.v1` and `task_management/delegation/deadline` at `k1/tools/family/tasks/definition.py` L84-L85, with `list_tasks` read action at L361-L362. Calendar profile/tags are `calendar.v1` and `scheduling/availability/external_calendar` at `k1/tools/family/calendar/definition.py` L103 and L126, with `list_events` read action at L271-L272.
- Back prompt already tells the model to use `batch_invoke_capabilities` for 2+ capabilities in `k1/concierge/prompt/back_prompt.py` L201, L324-L329, and L501-L504; this is guidance only and not sufficient as a scheduler.
- Back profile selection is metadata-driven in `k1/concierge/prompt/back_profiles.py` L115, with structured-domain selection at L288 and dedupe at L355.
- FSM/TaskBridge owns lifecycle and ledger writes: `k1/concierge/fsm/task_bridge.py` L6-L9, L49-L53, L215-L231, L277-L298, L334-L352, and L362-L385. Controller task complete/suspend handlers start at `k1/concierge/fsm/controller.py` L2742 and L3162.
- Ledger projections rebuild task state and pending results in `k1/concierge/ledger/projections.py` L324 and L449. Ledger writer append APIs are in `k1/concierge/ledger/writer.py` L62 and L104.
- The original V3 BackPool plan is in `k1/concierge/docs/v3_milestones.md` L9659, with the goal at L9661-L9666, gate at L9669-L9676, BackPool integration issue at L9713, BackTopicRouter integration issue at L9757, ReadyQueue completion issue at L9779, and cumulative wiring warning at L9824-L9839.
- Real production session creation enters at `k1/kernel/service.py` L2225, builds per-session mailboxes at L2247-L2255, calls `ConciergeFactory.create_with_ports` at L2505, starts the runtime at L2620, and stores only `consumer_task`/`concierge_task` in `SessionInstance` at L2655 and L2662. No BackPool object is constructed in this path.
- `ConciergeFactory._construct_concierge` starts at `k1/concierge/factory.py` L567, creates the FSM at L589, creates front/back `ToolContext` and the single `back_dispatcher` at L633-L663, and builds `ConciergeRuntime` at L803-L830. It does not create `BackPool`, `BackTopicRouter`, `ReadyQueue`, or call `fsm.set_back_pool`.
- `ConciergeRuntime` initializes a single `_back_task` slot in `k1/concierge/session.py` L96, cancels only that slot in `stop()` at L129-L135, reads the Back mailbox only when `_back_task is None` at L383-L388, creates exactly one Back task at L401, and routes directly through `route_back_envelope` at L408-L424.
- The production BackPool implementation already exists: `BackPoolConfig` at `k1/concierge/actors/back_pool.py` L42, `WorkerSlot` at L104, `BackPoolExhausted` at L145, `SessionLimitReached` at L167, `BackPool` at L186, `acquire_worker` at L230, `release_worker` at L350, `renew_lease` at L497, `start_lease_watcher` at L543, overflow helpers at L683-L713, cleanup at L727, and `get_pool_state` at L760.
- `BackTopicRouter` already exists at `k1/concierge/actors/back_router.py` L37, maps dispatch/resume/cancel/clarification topics at L70-L77, routes at L97, bypasses the pool for cancel at L146, discards late envelopes at L165, extracts lease tokens at L197, and extracts task IDs at L224.
- `ReadyQueue` already exists at `k1/concierge/actors/ready_queue.py` L42, enqueues dependencies at L102, drains ready envelopes at L209, and notifies completion at L228.
- `ConciergeController` has a BackPool attachment point at `k1/concierge/fsm/controller.py` L797-L806, reads BackPool utilization for weave signals at L1031-L1043, and defensively calls `release_worker` on completion at L2831 and L2875. ReadyQueue dependency release is runtime-owned in `ConciergeRuntime`, so the FSM task-complete path must not also dispatch dependent envelopes.
- Back handlers already accept lease cancellation tokens: `back_handler` at `k1/concierge/actors/back.py` L888, `back_resume_handler` at L1162, and `route_back_envelope` at L1571 with token-aware dispatch at L1629, L1643, L1655, and cancel at L1678.
- BackPool bus topics/builders already exist: `k1/concierge/bus/topics.py` L119-L121 and `k1/concierge/bus/builders.py` L505-L517. `TaskLease` and `CancellationToken` primitives exist at `k1/concierge/protocols/task_lease.py` L61 and L301 and `k1/concierge/protocols/cancellation.py` L55, L117, and L148.

**State/API ownership for M8:**

- Front owns user language, dispatch shaping, and final response style.
- Back owns execution coverage for the task payload it receives.
- ReAct owns model/tool iteration and Back execution checkpoints.
- Capability contracts own invocation safety facts: action kind, required inputs, side effects, and profile/domain metadata.
- FSM/TaskBridge owns task lifecycle and append-before-mutate ledger writes. M8 must not introduce direct Back writes to SessionState task sections or ledger lifecycle state.

**Epic M8.E1 -- Back Work Ledger And Intent Coverage**

### Issue M8.E1.I1 -- Introduce a Back execution work ledger

**Files to change:**

- `k1/concierge/react/back_execution_plan.py` (create)
- `k1/concierge/react/loop.py` L326, L510, L557, L637, L702, L1758-L1766, L1842-L1866
- `k1/concierge/actors/back.py` L509, L731, L776, L1111-L1130, L1460-L1481
- `k1/concierge/actors/frames.py` L124-L154, L158-L190
- `tests/k1/concierge/react/test_back_execution_plan.py` (create)
- `tests/k1/concierge/test_react_checkpoint.py` (update existing checkpoint expectations if M8 stores plan state in checkpoints)

**Implementation directive:**

- Add typed planner/work records for Back execution, for example `BackExecutionPlan`, `BackExecutionWorkItem`, and `BackExecutionStatus`.
- Each work item must keep `origin_intent_index`, `action`, `domain_hint`, `params`, `selected_capability`, `contract_kind`, `side_effects`, `required_inputs`, `status`, `result`, `error`, and `needs_human`/`blocker` if applicable.
- Seed the plan from the `TaskDispatch.intents` payload, not from the model's prose. Preserve bundled intent order from `k1/concierge/tools/schemas_front.py` L491-L518 and `k1/concierge/task/classifier.py` L115-L117.
- Update the plan from discovery payloads captured by `k1/concierge/react/loop.py` L1758-L1763 and authority attempts captured at L1765-L1766.
- Persist enough plan state in the ReAct checkpoint path (`k1/concierge/actors/back.py` L509, L1111-L1130, L1460-L1481) to resume after HIL without rediscovering completed work.
- Keep this as a Back/ReAct execution ledger, not the FSM lifecycle ledger. It must not call `TaskBridge.complete_task`, `TaskBridge.suspend_task`, or `LedgerWriter.append_sync` directly; those remain owned by `k1/concierge/fsm/task_bridge.py` L215-L385.

**Acceptance:** A Back task with two independent intents produces two work items, can mark one success and one blocked, and serializes/deserializes through checkpoint data without losing selected capability names or results.

**Run:** `pytest tests/k1/concierge/react/test_back_execution_plan.py tests/k1/concierge/test_react_checkpoint.py -v`

### Issue M8.E1.I2 -- Wire deterministic execution after discovery

**Files to change:**

- `k1/concierge/react/loop.py` L510, L557, L637, L702, L1628-L1654, L1663-L1697, L1758-L1766, L1842-L1866
- `k1/concierge/tools/implementations.py` L1401, L1558, L1733
- `tests/k1/concierge/react/test_back_capability_execution_kernel.py` (create)
- `tests/k1/concierge/react/test_front_dispatch_terminates.py` (update only if Back completion semantics affect existing termination probes)

**Implementation directive:**

- After Back discovers viable candidates for a work item, the kernel must either execute the selected capability, mark the item blocked by a concrete missing required input, or mark it ambiguous with candidate options. It must not rely on another LLM iteration to remember discovered candidates.
- Reuse or replace the existing collection helpers in `k1/concierge/react/loop.py` L510-L786; do not leave dead helpers with divergent behavior.
- Required-input-free read/list capabilities should execute automatically once selected and safety permits. This covers `tool.read.family_tasks.list_tasks` and `tool.read.calendar.list_events` style contracts.
- If multiple independent read work items are ready, build one batch path instead of serial model iterations.
- Preserve the model loop for genuinely ambiguous selection, missing required input HIL, or write approval decisions.

**Acceptance:** A fake model that only calls `discover_capabilities` for `list tasks` and `list calendar events` still yields a complete Back result with both intent results and does not hit `REACT_LOOP_DEGENERATE`.

**Run:** `pytest tests/k1/concierge/react/test_back_capability_execution_kernel.py -v`

### Issue M8.E1.I3 -- Gate `submit_result` on intent coverage

**Files to change:**

- `k1/concierge/react/loop.py` L1628-L1654
- `k1/concierge/tools/implementations.py` L2048-L2093
- `k1/concierge/tools/schemas_back.py` L207-L282
- `tests/k1/concierge/react/test_back_capability_execution_kernel.py` (extend)
- `tests/k1/concierge/test_back_result_frame.py` L73-L117

**Implementation directive:**

- Before accepting `submit_result(result_type="complete")`, verify that every Back work item is `succeeded`, `skipped_with_reason`, or `blocked_needs_human` only when `result_type="needs_human"`.
- If the model submits complete after only one of several bundled intents, feed a tool observation explaining the missing intent and allow one repair iteration.
- If budget is exhausted after partial work, return a typed partial/blocked frame rather than fabricating completion.
- Keep compatibility fields from `execute_submit_result` (`final_answer`, `results`, `artifacts_created`, `presentation_guidance`, `tool_call_summaries`) but make coverage state the authority.

**Acceptance:** A submit call with calendar results but no task result is rejected in-loop; a submit call with both intent facts is accepted and returned as complete.

**Run:** `pytest tests/k1/concierge/react/test_back_capability_execution_kernel.py tests/k1/concierge/test_back_result_frame.py -v`

### Issue M8.E1.I4 -- Emit per-intent typed BackResultFrame facts

**Files to change:**

- `k1/concierge/actors/back.py` L731-L805
- `k1/concierge/actors/frames.py` L124-L154, L158-L190, L198-L306
- `k1/concierge/actors/front.py` L410-L432, L1579
- `tests/k1/concierge/test_back_result_frame.py` L26-L43, L73-L117
- `tests/k1/concierge/test_front_present_frame.py` L21-L55
- `tests/k1/concierge/test_front_weave_frame.py` (update if frame shape changes)

**Implementation directive:**

- Back frame `facts` must be a structured array where each item can be traced to an intent/work item and capability result.
- Include provenance fields such as `intent_index`, `intent_action`, `capability_name`, `status`, `result`, and `source="capability"`.
- Keep `raw_final_answer` as compatibility only; Front PRESENT must be able to present from typed facts even when worker prose is empty or bland.
- Do not leak internal capability names to user-facing text by default; capability names are provenance for Front/policy, not visible copy.

**Acceptance:** Front PRESENT mode receives a Back frame with task facts and calendar facts, extracts both into scenario data, and final cleanup still removes raw worker frame leakage.

**Run:** `pytest tests/k1/concierge/test_back_result_frame.py tests/k1/concierge/test_front_present_frame.py tests/k1/concierge/test_front_weave_frame.py -v`

**Epic M8.E2 -- Contract-Aware Parallel And Batch Execution**

### Issue M8.E2.I1 -- Reconcile the two parallel policy modules

**Files to change:**

- `k1/concierge/task/parallel_safety.py` L40-L61, L84
- `k1/concierge/tools/parallelism.py` L22-L50, L58, L89
- `k1/concierge/react/loop.py` L43, L1663-L1697
- `k1/concierge/tools/__init__.py` if it exports the non-live helper
- `tests/k1/concierge/test_m03_e34_parallel_safety.py` L73-L109, L143-L168, L195-L243, L419-L564

**Implementation directive:**

- Make one module the production source of truth. The current live source is `k1/concierge/task/parallel_safety.py`; either move all richer partition behavior there or make `tools/parallelism.py` a thin compatibility wrapper over it.
- Remove the semantic conflict where `tools/parallelism.py` says `invoke_capability`, `spawn_via_fabric`, and `execute_workflow` can be parallel while live `task.parallel_safety` forces them sequential.
- Classification must understand wrapper tools: outer `invoke_capability` is not intrinsically safe or unsafe; the effective inner contract decides.
- `submit_result` and `dispatch_task` remain sequential control tools.

**Acceptance:** Tests prove the live ReAct loop and exported helper report the same partition for reads, writes, `invoke_capability`, `batch_invoke_capabilities`, `dispatch_task`, and `submit_result`.

**Run:** `pytest tests/k1/concierge/test_m03_e34_parallel_safety.py -v`

### Issue M8.E2.I2 -- Partition invocation batches by contract metadata

**Files to change:**

- `k1/concierge/task/parallel_safety.py` L40-L61, L84
- `k1/concierge/tools/implementations.py` L302-L331, L1558-L1682, L1733-L1841
- `k1/fabric/manifest_translator.py` L196, L209, L221
- `k1/tools/family/tasks/definition.py` L361-L362
- `k1/tools/family/calendar/definition.py` L271-L272
- `tests/k1/concierge/tools/test_batch_invoke_capabilities.py` (create)
- `tests/k1/concierge/test_m03_e34_parallel_safety.py` (extend)

**Implementation directive:**

- Add a contract-aware partition function that uses exact registry lookup and contract metadata: action kind/capabilities, required inputs, safety band, and `side_effects`.
- Read contracts with no side effects can run in parallel if they do not depend on each other and do not share an output-to-input edge.
- Write/delete/update contracts, workflow launches, HIL-gated operations, and unknown contracts stay sequential.
- Mixed batches must preserve input order in returned result arrays even when read items execute concurrently.

**Acceptance:** A batch containing `list_tasks` and `list_events` is classified parallel-safe; a batch containing `create_task` or `delete_event` is sequential or split into a read-parallel group followed by ordered writes.

**Run:** `pytest tests/k1/concierge/tools/test_batch_invoke_capabilities.py tests/k1/concierge/test_m03_e34_parallel_safety.py -v`

### Issue M8.E2.I3 -- Use dispatch/fabric batch execution instead of serial dispatch loops

**Files to change:**

- `k1/concierge/tools/implementations.py` L1733-L1906
- `k1/concierge/adapters/fabric_dispatch.py` L78, L363-L369
- `k1/fabric/fabric.py` L89, L727-L768, L1229, L1269, L1644
- `tests/k1/concierge/tools/test_batch_invoke_capabilities.py` (extend)
- `tests/k1/concierge/ports/test_dispatch_adapter.py` L58-L65
- `tests/k1/fabric/test_capability_versioning.py` or existing Fabric batch tests if present; otherwise create `tests/k1/fabric/test_fabric_execute_batch_ordering.py`

**Implementation directive:**

- Build `CapabilityRequest` objects first, after binding and param normalization.
- When all ready requests are read/no-side-effect, call `ctx.dispatch.execute_batch(..., strategy="PARALLEL")` if available.
- Update `FabricDispatchAdapter.execute_batch` so it delegates to Fabric's `execute_batch` when the underlying fabric exposes it instead of doing a Python list comprehension over `dispatch_direct`.
- Fall back to ordered sequential `dispatch_direct` only for adapters that truly lack batch support.
- Preserve per-item `duration_ms`, `retryable`, binding metadata, and result ordering.

**Acceptance:** A fake dispatch adapter records one `execute_batch` call for two read invocations and zero serial `dispatch_direct` calls; Fabric adapter delegates to Fabric `execute_batch` and returns results in input order.

**Run:** `pytest tests/k1/concierge/tools/test_batch_invoke_capabilities.py tests/k1/concierge/ports/test_dispatch_adapter.py tests/k1/fabric/test_fabric_execute_batch_ordering.py -v`

### Issue M8.E2.I4 -- Preserve timeout, budget, and HIL ordering semantics

**Files to change:**

- `k1/concierge/react/loop.py` L1117-L1162, L1663-L1697
- `k1/concierge/tools/dispatcher.py` L477-L587, L644-L650, L815-L820
- `k1/concierge/tools/implementations.py` L1733-L1906
- `tests/k1/concierge/test_m03_e34_parallel_safety.py` L195-L243, L419-L564
- `tests/k1/concierge/tools/test_tool_dispatcher_lax_limits.py` (update only if budget accounting changes)
- `tests/k1/concierge/tools/test_batch_invoke_capabilities.py` (extend)

**Implementation directive:**

- ReAct tool timeout wrapping in `loop.py` L1117-L1162 must apply to batch execution the same way it applies to single tool calls.
- One `batch_invoke_capabilities` call still costs one Back tool call at the dispatcher level, while internal per-capability failures remain visible in the batch result.
- HIL-gated actions must not be hidden inside a parallel batch. If any item requires approval/clarification, split it out and suspend with a typed blocker.
- Sequential write order must be stable and auditable.

**Acceptance:** Parallel read batch timeout returns a retryable per-batch or per-item error without hanging the loop; write batches execute in declared order; budget count remains one outer tool call.

**Run:** `pytest tests/k1/concierge/test_m03_e34_parallel_safety.py tests/k1/concierge/tools/test_batch_invoke_capabilities.py tests/k1/concierge/tools/test_tool_dispatcher_lax_limits.py -v`

**Epic M8.E3 -- Discovery Quality And Domain Robustness**

### Issue M8.E3.I1 -- Make domain hints soft and canonical

**Files to change:**

- `k1/fabric/retrieval/retrieval_engine.py` L166-L191, L224, L246-L297
- `k1/fabric/core/registry.py` L667-L680, L1251-L1253
- `k1/fabric/manifest_translator.py` L182-L184
- `k1/tools/family/tasks/definition.py` L84-L85
- `k1/tools/family/calendar/definition.py` L103, L126
- `tests/k1/fabric/test_retrieval_engine.py` L534-L625
- `tests/k1/fabric/test_registry_retrieval.py` (extend if registry aliasing is added there)

**Implementation directive:**

- `domain` in `discover_capabilities` must behave as a hint, not a hard kill switch. Scoped lookup may run first, but it must merge with an unscoped/global candidate pass when scoped candidates are empty or low quality.
- Canonicalization must be metadata-driven or configured centrally; do not add family-specific if/else routing inside ReAct or Back prompt code.
- A broad hint like `household` must not prevent `tasks.v1` or `calendar.v1` contracts from being retrieved when the intent text says tasks/events.
- Preserve the O(1) exact domain index optimization for precise tags, but do not let exact misses eliminate all candidates for broad/composite user language.

**Acceptance:** `discover_capabilities(intent="list tasks", domain=["household"])` returns the tasks read contract above unrelated calendar/date/unit contracts; `discover_capabilities(intent="list calendar events", domain=["household"])` still returns calendar reads.

**Run:** `pytest tests/k1/fabric/test_retrieval_engine.py tests/k1/fabric/test_registry_retrieval.py -v -k "domain or discover"`

### Issue M8.E3.I2 -- Add retrieval quality gates for action/adapter evidence

**Files to change:**

- `k1/fabric/retrieval/retrieval_engine.py` L320-L394
- `k1/fabric/retrieval/soft_ranker.py` L9-L18, L185-L233, L252-L303, L335-L340
- `k1/fabric/retrieval/hard_filter.py` L89-L106, L200-L236, L289-L300, L335-L360
- `k1/fabric/manifest_translator.py` L192-L196, L221
- `tests/k1/fabric/test_retrieval_engine.py` L367-L374, L534-L625

**Implementation directive:**

- Retrieval ranking must consider action/kind/adapter evidence from contracts, not only semantic embedding plus domain Jaccard.
- For a `list tasks` query, candidates without task adapter/profile/action evidence must not outrank a tasks read contract merely because they are globally common reads.
- For multi-intent discovery, each intent gets its own candidate quality gate; do not let calendar candidates satisfy tasks work items.
- Report candidate diagnostics in discovery output so Back can see why a result was selected or rejected.

**Acceptance:** A test registry containing tasks, calendar, date, and unit contracts ranks `tool.read.family_tasks.list_tasks` first for `list tasks`, and ranks `tool.read.calendar.list_events` first for `list calendar events`.

**Run:** `pytest tests/k1/fabric/test_retrieval_engine.py -v -k "rank or discover"`

### Issue M8.E3.I3 -- Align Back profile/domain selection with bundled intents

**Files to change:**

- `k1/concierge/prompt/back_profiles.py` L115-L125, L244-L256, L282-L314, L355-L426
- `k1/concierge/actors/back.py` L453, L1007, L1351
- `k1/concierge/tools/implementations.py` L1264-L1381
- `tests/k1/concierge/prompt/test_back_profiles.py`
- `tests/k1/concierge/prompt/test_back_prompt_profile_injection.py`
- `tests/k1/concierge/tools/test_task_dispatch_execution_profiles.py`
- `tests/k1/concierge/test_back_handler_profile_wiring.py`

**Implementation directive:**

- Bundled intents can require multiple profiles, for example `tasks.v1` plus `calendar.v1`. Do not collapse the task to a single broad domain such as `household` when the payload carries separate intent actions/domains.
- Preserve the existing metadata-driven profile selection. Do not introduce text-scanning cue tables in Back.
- If the Front dispatch contains explicit `execution_profiles`, reuse them; otherwise derive at most the configured profile limit from structured intent/domain metadata and capability contracts.
- Feed profile/domain metadata into discovery as per-intent hints, not as a task-wide hard filter.

**Acceptance:** A bundled tasks+calendar task selects both relevant profiles when metadata supports it and does not select unrelated shopping/reminders profiles.

**Run:** `pytest tests/k1/concierge/prompt/test_back_profiles.py tests/k1/concierge/prompt/test_back_prompt_profile_injection.py tests/k1/concierge/tools/test_task_dispatch_execution_profiles.py tests/k1/concierge/test_back_handler_profile_wiring.py -v`

### Issue M8.E3.I4 -- Keep member references user-facing and normalized internally

**Files to change:**

- `k1/concierge/prompt/back_prompt.py` L285, L407-L408
- `k1/concierge/tools/implementations.py` L244-L302, L1567, L1775
- Family tool definitions with person/member fields as needed
- `tests/k1/concierge/prompt/test_back_prompt_capability_names.py` L85-L98
- Existing member-normalization tests; if no focused file exists, create `tests/k1/concierge/tools/test_family_member_reference_normalization.py`

**Implementation directive:**

- Back must never ask the user for internal family member IDs. It may pass human names or normalized slugs through capability params, and implementation code normalizes member reference fields before dispatch.
- Batch invocation must apply the same normalization as single invocation.
- If a capability truly cannot resolve a member reference, the HIL question asks for the family member by name/relationship, not for a database/member ID.

**Acceptance:** `Riley` in single and batch capability params normalizes to the expected slug internally, and no Back prompt or HIL path asks "What is Riley's member ID?".

**Run:** `pytest tests/k1/concierge/prompt/test_back_prompt_capability_names.py tests/k1/concierge/tools/test_family_member_reference_normalization.py -v`

**Epic M8.E4 -- End-To-End Regression Probes And Ledger Boundaries**

### Issue M8.E4.I1 -- Add a deterministic Back regression for tasks plus calendar

**Files to change:**

- `tests/k1/concierge/react/test_back_capability_execution_kernel.py` (extend)
- `tests/integration/k1/live/m1/test_m1_x3_x12_concierge_cross_component.py` (add only if a live/fake bus probe is needed)
- `k1/concierge/actors/back.py` L888, L1007, L1070, L1129-L1130
- `k1/concierge/react/loop.py` L1628-L1697, L1758-L1766
- `k1/concierge/fsm/controller.py` L2742, L2823, L2894, L3162, L3280

**Implementation directive:**

- Build a fake dispatch payload equivalent to one Front `dispatch_task` with two bundled intents: list tasks and list calendar events.
- Fake discovery should return one tasks read and one calendar read. Fake dispatch should return deterministic records for both.
- Assert Back emits one complete typed frame, with two per-intent facts, no missing submit, and no degenerate loop status.
- If implemented at bus/FSM level, assert FSM receives task complete through normal `TOPIC_TASK_COMPLETE` handling and TaskBridge owns lifecycle mutation.

**Acceptance:** The regression fails on the current partial-calendar-only behavior and passes only when both tasks and calendar results are present in the Back frame.

**Run:** `pytest tests/k1/concierge/react/test_back_capability_execution_kernel.py -v -k "tasks_calendar"`

### Issue M8.E4.I2 -- Prove lifecycle ledger ownership is unchanged

**Files to change:**

- `k1/concierge/fsm/task_bridge.py` L215-L231, L277-L298, L334-L352, L362-L385
- `k1/concierge/ledger/projections.py` L324-L449
- `k1/concierge/ledger/writer.py` L62, L104
- `k1/concierge/react/back_execution_plan.py` (created by M8.E1.I1)
- `tests/k1/concierge/test_m01_ledger_replay.py` L72-L177, L239
- `tests/k1/concierge/ledger/test_m5_g5_ledger_recovery_at_set_session_state.py` L76-L143
- `tests/k1/concierge/react/test_back_execution_plan.py` (extend)

**Implementation directive:**

- M8's Back execution plan can checkpoint work, but it must not append lifecycle ledger events or mutate TaskBridge state.
- Task complete/suspended/resumed events must still flow through existing bus/FSM handlers and TaskBridge append-before-mutate behavior.
- Ledger replay should not need to understand transient per-intent execution details unless those details are already carried as task result payloads.

**Acceptance:** Unit tests spy on Back execution plan operations and assert no calls to `LedgerWriter.append_sync`, `TaskBridge.complete_task`, or `TaskBridge.suspend_task`; existing ledger replay tests remain green.

**Run:** `pytest tests/k1/concierge/react/test_back_execution_plan.py tests/k1/concierge/test_m01_ledger_replay.py tests/k1/concierge/ledger/test_m5_g5_ledger_recovery_at_set_session_state.py -v`

### Issue M8.E4.I3 -- Add user-facing presentation leak probes

**Files to change:**

- `k1/concierge/actors/front.py` L288, L410-L432, L1579
- `k1/concierge/actors/frames.py` L198-L306, L397
- `tests/k1/concierge/test_front_present_frame.py` L21-L55
- `tests/k1/concierge/test_front_weave_frame.py`
- `tests/k1/concierge/test_m7_e1_one_brain_illusion.py` (extend if M7 file exists)

**Implementation directive:**

- Add probes where Back facts contain capability provenance, member slugs, and raw worker prose. Front output must include the useful user facts and exclude raw frame JSON, internal member IDs, registry slugs, and tool names unless explicitly requested for diagnostics.
- Verify typed facts, not `raw_final_answer`, drive the response.

**Acceptance:** Final response contains the task and calendar information, does not contain `_frame_type`, `capability_name`, `tool.read`, `member_id`, raw JSON keys, or `REACT_LOOP_DEGENERATE`.

**Run:** `pytest tests/k1/concierge/test_front_present_frame.py tests/k1/concierge/test_front_weave_frame.py -v`

### Issue M8.E4.I4 -- Add an implementation closeout matrix

**Files to change:**

- `docs/architecture/kernel_sweep_status.md`
- `k1/concierge/OPEN_ISSUES.md` only if an M8 issue is intentionally deferred
- `docs/plans/k1/conceriege_strengthning_plan.md` (this section, update line anchors after code movement)

**Implementation directive:**

- Add an M8 closeout table with rows for M8.E1.I1-I4, M8.E2.I1-I4, M8.E3.I1-I4, M8.E4.I1-I4, and M8.E5.I1-I4.
- Each row must record production files changed, tests added/updated, and whether behavior is green, strict xfail, or deferred.
- Update the line anchors in this plan after implementation, because code edits will move current line numbers.

**Acceptance:** M8 implementation cannot be called complete until every issue has either a green focused test or a named strict xfail/deferred issue with a reason.

**Run:** `pytest tests/k1/concierge/react/test_back_execution_plan.py tests/k1/concierge/react/test_back_capability_execution_kernel.py tests/k1/concierge/tools/test_batch_invoke_capabilities.py tests/k1/fabric/test_retrieval_engine.py tests/k1/concierge/test_back_result_frame.py tests/k1/concierge/test_front_present_frame.py -v`

**Status:** M8 closeout matrix is green in `docs/architecture/kernel_sweep_status.md`. M8.E1-M8.E4 are covered by the capability execution, discovery, presentation, and lifecycle regressions; M8.E5 is covered by `tests/k1/concierge/test_m8_e5_backpool_runtime_wiring.py`, including factory/runtime wiring, KernelService Tier-2 wiring, overlapping workers, overflow, cancel bypass, resume/clarification routing, FSM-to-lease CancellationToken identity, lease watcher lifecycle, ReadyQueue dependency failure, and BackPool observability.

**Epic M8.E5 -- BackPool Runtime Worker Wiring**

### Issue M8.E5.I1 -- Construct BackPool in the real k1 session path

**Files to change:**

- `k1/concierge/config/kernel.py` L80-L85
- `k1/concierge/config/concierge.py` L19-L42, L54-L69
- `k1/concierge/config/loader.py` L159-L160, L886-L889
- `k1/concierge/config/defaults.yaml` L205-L212
- `k1/kernel/service.py` L2225, L2247-L2255, L2505-L2520, L2620, L2655, L2662
- `k1/concierge/factory.py` L567, L589, L633-L663, L803-L830
- `k1/concierge/session.py` L43-L96, L108-L135
- `k1/concierge/fsm/controller.py` L797-L806, L1031-L1043
- `k1/concierge/actors/back_pool.py` L42-L97, L186-L230, L760-L789
- `k1/concierge/actors/back_router.py` L37-L77
- `k1/concierge/actors/ready_queue.py` L42-L102
- `tests/k1/concierge/test_m8_e5_backpool_runtime_wiring.py` (create)
- `tests/k1/kernel/test_service.py` (extend only if new KernelConfig fields are surfaced there)

**Implementation directive:**

- Add production config fields for BackPool sizing and leases using the defaults already encoded by `BackPoolConfig`: pool size 3, max concurrent per session 2, lease TTL 300 seconds, reclaim interval 30 seconds, dependency ordering enabled, max renewals 3, and lease grace 5 seconds.
- Thread those fields from KernelConfig to ConciergeConfig, then build one per-session `BackPool`, one `BackTopicRouter(back_pool)`, and one `ReadyQueue` in `ConciergeFactory._construct_concierge` or in a small helper called by it.
- Pass the pool/router/ready-queue into `ConciergeRuntime`; expose read-only properties for tests and observability.
- Call `fsm.set_back_pool(back_pool)` before runtime start so `WeaveSignal.from_runtime` and completion release hooks observe the same pool instance.
- Do not construct the pool in both `KernelService` and `ConciergeFactory`. Choose one ownership boundary; the factory is preferred because it already owns runtime composition.

**Acceptance:** A `KernelService._create_session_tier2` session creates a runtime whose `back_pool`, `back_topic_router`, and `ready_queue` are non-null, whose FSM points at the same pool, and whose BackPool defaults match the V3 plan unless overridden by config.

**Status:** Implemented. `ConciergeFactory` owns per-session BackPool construction and `KernelService._create_session_tier2` reaches that same runtime wiring through `ConciergeConfig.from_kernel_config`.

**Run:** `pytest tests/k1/concierge/test_m8_e5_backpool_runtime_wiring.py tests/k1/kernel/test_service.py -v -k "backpool or KernelConfig"`

### Issue M8.E5.I2 -- Replace the single `_back_task` slot with pool scheduling

**Files to change:**

- `k1/concierge/session.py` L96, L129-L135, L334-L401, L408-L424
- `k1/concierge/actors/back_pool.py` L230-L350, L350-L408, L683-L713, L727-L760
- `k1/concierge/actors/back_router.py` L97-L146, L165-L224
- `k1/concierge/actors/back.py` L888, L1162, L1571-L1680
- `k1/concierge/protocols/task_lease.py` L61-L129, L301-L341
- `k1/concierge/protocols/cancellation.py` L55-L130, L148
- `tests/k1/concierge/test_m8_e5_backpool_runtime_wiring.py` (extend)
- `tests/k1/concierge/test_m03_e36_conformance.py` (extend only for router semantics)
- `tests/k1/concierge/test_back_handler_profile_wiring.py` (ensure worker path keeps profile/HIL wiring)

**Implementation directive:**

- Remove `_back_task` as the concurrency gate. The runtime may keep a set/dict of active worker asyncio tasks for shutdown, but BackPool is the authority for slot ownership.
- Each mailbox poll must try to schedule as many ready Back envelopes as pool capacity permits, not only one envelope when `_back_task is None`.
- Route every Back envelope through `BackTopicRouter`. `task.cancel.v1` must run synchronously and consume no pool slot. `task.dispatch.v1`, `task.resume.v1`, and `clarification.response.v1` acquire a worker slot and bind the created asyncio task to the returned `WorkerSlot`.
- Pass the lease `CancellationToken` to `back_handler` and `back_resume_handler` through the existing `cancel_token` parameter. Keep `route_back_envelope` as compatibility if useful, but do not double-route in a way that bypasses `BackTopicRouter` late-envelope discard or cancel-token extraction.
- Release workers on success, cancellation, suspension, and error with distinct reasons. Suspension must free the slot without causing the later resume envelope to be discarded.

**Acceptance:** Two Back `task.dispatch.v1` envelopes in the same session can overlap when `pool_size >= 2`; a third envelope is queued when the pool is full; a cancel envelope for a running task is processed immediately without waiting for a slot; resume and clarification envelopes hit `back_resume_handler`.

**Status:** Implemented. `ConciergeRuntime` now uses BackPool scheduling and active worker task tracking instead of the single `_back_task` gate; tests cover overlap, overflow, cancel bypass, resume/clarification routing, and the single CancellationToken identity from FSM to TaskLease to Back handler.

**Run:** `pytest tests/k1/concierge/test_m8_e5_backpool_runtime_wiring.py tests/k1/concierge/test_m03_e36_conformance.py tests/k1/concierge/test_back_handler_profile_wiring.py -v`

### Issue M8.E5.I3 -- Wire leases, overflow drain, ReadyQueue, and observability

**Files to change:**

- `k1/concierge/session.py` L108-L135, L334-L401
- `k1/concierge/fsm/controller.py` L797-L806, L1031-L1043, L2742-L2762, L2829-L2877, L2899-L2900, L2976
- `k1/concierge/actors/back_pool.py` L497-L530, L543-L680, L683-L713, L760-L789
- `k1/concierge/actors/ready_queue.py` L102-L209, L228-L289
- `k1/concierge/bus/topics.py` L119-L121, L348-L350
- `k1/concierge/bus/builders.py` L505-L517, L616-L618, L681-L683
- `tests/k1/concierge/test_m8_e5_backpool_runtime_wiring.py` (extend)
- `tests/k1/concierge/test_m08_e81_weave_signal.py` (extend if pool utilization wiring changes)
- `tests/k1/concierge/test_m00_v3_conformance.py` (extend only if guard-table/event assertions need k1 BackPool coverage)

**Implementation directive:**

- Start `BackPool.start_lease_watcher(session_bus)` in `ConciergeRuntime.start()` and stop it in `ConciergeRuntime.stop()`. Runtime shutdown must cancel/await all active worker tasks, not only the old single `_back_task`.
- Emit `k1.backpool.task.leased.v1`, `k1.backpool.worker.acquired.v1`, and `k1.backpool.worker.released.v1` from the live acquisition/release path using the existing bus builders. These are observability events; they must not mutate task lifecycle state.
- On `BackPoolExhausted` or `SessionLimitReached`, enqueue the envelope in BackPool overflow and retry from overflow before reading more mailbox work. Avoid a busy loop when the pool stays full.
- Extract `depends_on` from `task.dispatch.v1` payloads before worker acquisition. Enqueue through `ReadyQueue` when dependency ordering is enabled; only acquire workers for `dequeue_ready()` envelopes.
- Wire completion notification exactly once. Either the runtime subscribes to completion/failure/cancel topics and calls `ReadyQueue.notify_completed`, or the FSM invokes a runtime-owned scheduling callback after TaskBridge mutation. Do not let both paths dispatch the same dependent envelope.
- Failed/cancelled predecessors must produce dependency-failed outcomes for waiting tasks instead of starting another Back LLM that will rediscover a missing prerequisite.
- Preserve the TaskBridge/ledger single-writer invariant: BackPool and ReadyQueue schedule workers and emit observability/failure envelopes, but they do not directly mutate SessionState task lifecycle sections.

**Acceptance:** Lease expiry cooperatively cancels then reclaims a hung worker and emits `task.failed`; overflow drains when a worker releases; a dependent Back task starts only after its predecessor completes; a failed predecessor fails dependents without invoking Back; `WeaveSignal.from_runtime` still reads post-release pool utilization.

**Status:** Implemented for runtime wiring. Runtime starts/stops the BackPool lease watcher, drains overflow through ReadyQueue, binds the FSM CancellationToken into each TaskLease, emits BackPool observability events, and fails dependency waiters without invoking Back.

**Run:** `pytest tests/k1/concierge/test_m8_e5_backpool_runtime_wiring.py tests/k1/concierge/test_m08_e81_weave_signal.py tests/k1/concierge/test_m00_v3_conformance.py -v -k "backpool or ready_queue or lease or weave"`

### Issue M8.E5.I4 -- Promote POC BackPool tests to k1 production regressions

**Files to change:**

- `tests/k1/concierge/test_m8_e5_backpool_runtime_wiring.py` (create/extend)
- `tests/k1/concierge/test_m03_e36_conformance.py` (extend)
- `tests/k1/concierge/test_back_handler_profile_wiring.py` (extend)
- `tests/k1/concierge/test_m08_e81_weave_signal.py` (extend)
- `tests/k1/concierge/test_m00_v3_conformance.py` (extend if needed)
- POC reference sources only: `tests/poc/test_m07_e75_wiring.py`, `tests/poc/test_m07_e74_ready_queue.py`, `tests/poc/test_m07_e72_task_lease.py`, and `tests/poc/test_m03_e31_back_router.py`

**Implementation directive:**

- Port the intent of the POC tests into `tests/k1/concierge`; do not import from `poc.k1_poc` in production tests.
- Add a runtime-level fake mailbox/model regression proving two Back envelopes overlap under `ConciergeRuntime`, not merely that `BackPool.acquire_worker` works as a standalone class.
- Add focused tests for pool exhaustion and overflow FIFO drain, session limit, synchronous cancel/no slot, resume/clarification routing, lease watcher reclaim, ReadyQueue dependency release/failure/cycle handling, observability topic emission, and worker release on complete/error/suspend.
- Include one service/factory regression proving `KernelService._create_session_tier2` reaches the same BackPool wiring used by unit-level `ConciergeFactory` tests.
- Keep existing Back handler profile, HIL, typed-frame, and weave tests green; BackPool wiring must not regress profile rebinding, HIL suspension/resume, or frame presentation.

**Acceptance:** k1 tests fail on the current single `_back_task` implementation and pass only when the production runtime actually uses BackPool/BackTopicRouter/ReadyQueue. POC-only coverage no longer counts as evidence that real k1 Concierge has multiple Back workers.

**Status:** Implemented. POC intent is represented by `tests/k1/concierge/test_m8_e5_backpool_runtime_wiring.py`; no production regression imports from `poc.k1_poc`.

**Run:** `pytest tests/k1/concierge/test_m8_e5_backpool_runtime_wiring.py tests/k1/concierge/test_m03_e36_conformance.py tests/k1/concierge/test_back_handler_profile_wiring.py tests/k1/concierge/test_m08_e81_weave_signal.py -v`

**Run for M8:** `pytest tests/k1/concierge/react/test_back_execution_plan.py tests/k1/concierge/react/test_back_capability_execution_kernel.py tests/k1/concierge/tools/test_batch_invoke_capabilities.py tests/k1/concierge/test_m03_e34_parallel_safety.py tests/k1/fabric/test_retrieval_engine.py tests/k1/concierge/test_back_result_frame.py tests/k1/concierge/test_front_present_frame.py tests/k1/concierge/prompt/test_back_prompt_capability_names.py tests/k1/concierge/test_m8_e5_backpool_runtime_wiring.py -v`

**Test discipline:** Start with M8.E5.I1-I2 plus M8.E1.I1 and M8.E3.I1 focused tests. Do not run broad live suites until the deterministic fake Back regression is green and the runtime-level BackPool test proves two Back envelopes can overlap. Use live cross-component probes only after the fake execution kernel test proves both intents are covered and the runtime worker-pool path is active.
...
9. **M8:** Back capability execution kernel, contract-aware batching, and discovery quality gates.

My strongest recommendation for the original hardening train: start with **M1 + M3**. HIL unification fixes the most visible split-brain behavior, and ReAct checkpoint/control events make every later enhancement safer.

For the current Back capability-execution failure, start M8 with **M8.E5.I1-I2 + M8.E1 + M8.E3** after confirming the M2/M3 primitives are present in the branch. Then wire **M8.E2** batch execution once intent coverage, discovery quality, and the real BackPool runtime path are locked.
