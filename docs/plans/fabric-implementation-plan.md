...
> **Wiring**:
>
> - **AgentSpecValidator** (4.5.1) depends on Registry.lookup() (2.2.3) to verify tools_granted[] exist, ContractValidator (2.1.1) for base validation logic. No new ports.
> - **build_agent tool** (4.5.2) depends on AgentSpecValidator (4.5.1), AgentComposer (4.5.4), Registry.register() (2.2.2), IEventPort (5.1.2) for emit_agent_created. **Executed by Orchestrator** (not Planner) per PLAN-06.
> - **Discovery tool wrappers** (4.5.3) expose FabricRetrieval (5.3.3) as MCP tools. Require ToolContractParser (2.1.2) to create tool contracts.
> - **AgentComposer** (4.5.4) uses builder pattern, validates incrementally, consumed by build_agent tool (4.5.2).
> - **Meta-operation policy** (4.5.5) extends SecurityContext (3.2.1). Adds recursive creation prevention and agent-to-agent invocation block (depth=1).
> - **Lifecycle** (4.5.6) extends CapabilityContract (1.3.3) with new fields. Extends Registry (2.2.1) with created_agents index. ModuleLoader (2.3.1) for YAML persistence.
> - **Events** (4.5.7) adds 3 new event types to EventEmitter (5.4.2). Consumed by any subscriber on K1 Event Bus.
> - **AgentResponsePayload** (4.5.8) adds to types.py (1.3.3) as new frozen dataclass. Standard envelope for specialist agent output.
> - **ParamResolver** (4.5.9) extends Orchestrator's existing ParamResolver. Resolves dynamic capability names from prior step results. Critical for meta-agent DAG execution.
> - **Depends on**: Registry (2.2), ContractValidator (2.1.1), SecurityContext (3.2.1), IEventPort (5.1.2), FabricRetrieval (5.3.3), types.py (1.3.3).
> - **Consumed by**: Orchestrator DAG Executor (via build_agent as a DAG step), Planner (read-only discovery tools), Concierge (AgentResponsePayload consumption).
> - **Phase 2 deferred to M9**: Runtime prompt template creation (killed in M4), ORCH-13 micro-replan for mid-DAG agent discovery, K0 integration, workflow creation tools, template recommendation.
...
> - **Meta-agent creation** (6.3.15) verifies full Epic 4.5 workflow: discovery -> build_agent -> registration -> execution -> ephemeral cleanup.
> - **Recursive prevention** (6.3.16) verifies SecurityContext meta-operation gates (4.5.5): no recursive creation, no safety escalation, depth=1.
> - **AgentResponsePayload** (6.3.17) verifies standard agent output envelope (4.5.8): serialization, domain_data freeform, Concierge consumption.
