...
| M4 | Retrieval + Context + Agent Factory | 4 | Semantic search, context builder, agent factory, fabric mailbox/concurrency |
...
## Milestone 4: Retrieval + Context + Agent Factory

> **Goal**: Implement Semantic Retrieval Engine (Subsystem 3), Context Builder (Subsystem 7), and Agent Factory. This completes all three Fabric Roles.
>
> **Build Order Reference**: Phase 5 (Context Builder), Phase 6 (Retrieval), Phase 8 (Agent Factory).
>
...
| Issue | Title | Status | Deliverable | Details |
|-------|-------|--------|-------------|---------|
| 4.3.1 | Implement AgentFactory | TODO | k1/fabric/providers/agent_provider.py | 8-step instantiation: (1) Load YAML template, (2) Create MPSC Mailbox (WFQ INTERACTIVE priority), (3) Grant LLM access via IModelGatewayPort, (4) Grant SessionState read access (declared sections only, lock-free), (5) Scope tool access (tools_granted from contract, override from plan step, strict enforcement), (6) Build initial context via ContextBuilder, (7) Instantiate Agent object, (8) Start lifecycle: PENDING -> WARMING -> ACTIVE -> execute -> IDLE/DRAINING -> TERMINATED. |
| 4.3.2 | Implement Agent class | TODO | k1/fabric/providers/agent_provider.py | Agent object: id (UUID), contract (AgentContract), mailbox, llm_handle, state_reader, tool_scope, context, lifecycle_state. Methods: `warm_up()`, `execute(params) -> CapabilityResult`, `drain()`, `terminate()`. Lifecycle: PENDING -> WARMING (model preload) -> ACTIVE (executing) -> IDLE (pool, 60s TTL) -> DRAINING -> TERMINATED. |
| 4.3.3 | Implement Agent Pool (reuse) | TODO | k1/fabric/providers/agent_provider.py | IDLE pool for agent reuse. When a new task of the same type arrives within idle TTL (60s), reactivate from pool instead of cold start. Pool keyed by contract name. Max pool size configurable. |
| 4.3.4 | Implement Delta emission pattern | TODO | k1/fabric/providers/agent_provider.py | Agents NEVER write SessionState (FAB-01). Instead: agent emits delta to Delta Bus `k1.agent.{agent_id}.delta.v1` -> Aggregation Window (500ms batching, LWW merge) -> Concierge -> MutationGuard -> SessionState. Implement delta emission via IDeltaBusPort. |
| 4.3.5 | Complete AgentProvider (full) | TODO | k1/fabric/providers/agent_provider.py | Replace M3 stub. Full implementation using AgentFactory + Agent class. Handle: template not found, model loading failure, tool scope violation, execution timeout, clean resource disposal on terminate. |
...
| Issue | Title | Status | Deliverable | Details |
|-------|-------|--------|-------------|---------|
| 4.4.1 | Implement FabricMailbox | TODO | k1/fabric/core/fabric_mailbox.py | MPSC mailbox for Fabric request ingestion. WFQ (Weighted Fair Queuing) with REALTIME priority class. Accepts CapabilityRequest messages. Backpressure: when queue depth exceeds threshold, reject with `fabric_overloaded` error. Config: max_queue_depth (default 1000), priority_weights (REALTIME: 4, INTERACTIVE: 2, BACKGROUND: 1). Thread-safe, async-compatible. |
| 4.4.2 | Implement request scheduling and backpressure | TODO | k1/fabric/core/fabric_mailbox.py | Request scheduler: dequeue by WFQ priority order. Backpressure signals: (1) `queue_depth > 80%` -> emit `k1.fabric.pressure.warning.v1`, (2) `queue_depth > 95%` -> start rejecting BACKGROUND tier, (3) `queue_depth = 100%` -> reject all new requests. Integrate with FabricFacade: requests enter via mailbox, not direct method call in production mode. Standalone mode can bypass mailbox for simplicity. |
...
| Issue | Title | Status | Deliverable | Details |
|-------|-------|--------|-------------|---------|
| 5.1.1 | Define ISessionStateReader port | TODO | k1/fabric/ports/state_reader.py | Read-only access to SessionState for Context Builder. Methods: `read_section(section_name) -> dict | None`,`read_sections(names: list[str]) -> dict[str, Any]`,`get_snapshot() -> SessionSnapshot`. Multi-reader, lock-free. NEVER writes (FAB-01 enforcement). |
| 5.1.2 | Define IEventPort port | TODO | k1/fabric/ports/event_port.py | Event emission for all Fabric events. Methods: `emit(topic: str, payload: dict)`, `subscribe(topic: str, handler: Callable)`, `unsubscribe(subscription_id: str)`. All events carry cognitive_trace_id (FAB-09). Reuse interface pattern from SessionState IEventPort. |
| 5.1.3 | Define IBridgePort port | TODO | k1/fabric/ports/bridge_port.py | K0 access for Bridge Provider via BridgeConnectionAdapter. Methods: `send_command(operation: str, payload: dict) -> dict`, `is_available() -> bool`, `get_health() -> BridgeHealth`, `route_ifl(address: str, payload: dict) -> dict` (IFL-routed tool execution from bridge_architecture.mmd). **Memory operations**: memory.store, memory.recall, memory.delta, checkpoint, feedback.signal. **IFL operations**: tool.execute.home.*, tool.execute.device.* -- routed through Bridge's Inter-Function-Language addressing. Handles K0 offline gracefully (is_available() returns False, all operations return fallback). |
| 5.1.4 | Define IModelGatewayPort port | TODO | k1/fabric/ports/model_gateway.py | LLM access for Agent Factory. Methods: `create_handle(budget_tokens: int, model_preference: str or None, capabilities: list[str] or None, trace_id: str) -> LLMHandle`, `is_model_loaded(model_id: str) -> bool`, `list_capabilities() -> list[ModelCapability]`, `find_model(required_capabilities: list[str]) -> str or None`. **Capability-driven routing**: Model Hub has manifests with capabilities (CHAT, TOOL_CALL, STRUCTURED, EMBED, VISION, BATCH). Port interface must support capability-based model selection, not just model_preference string. Per-provider circuit breakers handled by Model Hub internally. Agent Factory grants LLM handles to spawned agents through this port. |
| 5.1.5 | Define IPromptSystemPort port | TODO | k1/fabric/ports/prompt_system.py | Prompt resolution + compilation for Context Builder. Methods: `resolve(template_name: str) -> PromptTemplate`, `compile(template: PromptTemplate, variables: dict) -> str`. Used by ContextBuilder step 4 (resolve prompt template). **NOTE**: The Prompt Management System is a full peer subsystem in the K1 architecture skeleton. Fabric defines the PORT INTERFACE ONLY. The Prompt System implementation (template storage, versioning, A/B testing, compilation engine) is OUT OF SCOPE for this plan and will be a separate module. Fabric consumes it through this port. |
| 5.1.6 | Define IDeltaBusPort port | TODO | k1/fabric/ports/delta_bus.py | Delta emission for agents. Methods: `emit_delta(agent_id: str, delta_type: str, section: str, data: dict)`. Used by Agent class to emit discovered facts/state changes that should be written to SessionState by Concierge. |
...
| Issue | Title | Status | Deliverable | Details |
|-------|-------|--------|-------------|---------|
| 5.2.1 | SessionStateReaderAdapter (production) | TODO | k1/fabric/adapters/sessionstate_reader.py | Wraps real SessionState (`SessionStateFactory.create_for_testing()` or wired manager). Implements ISessionStateReader. Multi-reader lock-free reads. |
| 5.2.2 | TestSessionStateReaderAdapter (test) | TODO | k1/fabric/adapters/test_state_reader.py | In-memory dict-based SessionState stub. Pre-load sections for testing. No real SessionState dependency. Implements ISessionStateReader. |
| 5.2.3 | LocalEventAdapter (reuse from SessionState) | TODO | k1/fabric/adapters/local_event.py | Reuse SessionState's LocalEventAdapter with capture_mode. Implements IEventPort. capture_mode=True for testing (stores all events). |
| 5.2.4 | TestBridgeAdapter | TODO | k1/fabric/adapters/test_bridge.py | In-memory K0 stub. Returns canned responses for memory.recall, memory.store. is_available() configurable (default True). Implements IBridgePort. |
| 5.2.5 | TestModelGatewayAdapter | TODO | k1/fabric/adapters/test_model_gateway.py | Returns canned LLM responses. create_handle() returns a TestLLMHandle that responds with configurable text. No real LLM dependency. Implements IModelGatewayPort. |
| 5.2.6 | TestPromptSystemAdapter | TODO | k1/fabric/adapters/test_prompt_system.py | Returns static prompt templates from in-memory dict. resolve() looks up by name. compile() does simple string replacement. Implements IPromptSystemPort. |
| 5.2.7 | TestDeltaBusAdapter | TODO | k1/fabric/adapters/test_delta_bus.py | Capture mode: stores all emitted deltas. drain() returns + clears. Used for testing agent delta emission. Implements IDeltaBusPort. |
| 5.2.8 | BridgeConnectionAdapter (production) | TODO | k1/fabric/adapters/bridge_connection.py | Production adapter for IBridgePort. Connects to Cross-Kernel Bridge WHEN AVAILABLE. Bridge design is complete (`bridge_architecture.mmd`) but Bridge code is NOT yet built. Adapter pattern: (1) On init: attempt connection to Bridge endpoint, (2) `is_available()`: returns True only if Bridge is connected and healthy, (3) All operations: if available, route through Bridge; if not, return graceful fallback (LOCAL COLD mode). (4) Auto-reconnect: periodic retry if Bridge becomes available later. This allows Fabric to be developed and tested independently while being Bridge-ready from day one. Implements IBridgePort. |
...
| Issue | Title | Status | Deliverable | Details |
|-------|-------|--------|-------------|---------|
| 5.3.1 | Implement FabricFactory | TODO | k1/fabric/factory.py | Factory methods: `create_standalone()` (all test adapters, no external deps), `create_for_testing()` (capture mode events + test adapters), `create_with_ports(state_reader, event_port, bridge, model_gateway, prompt_system, delta_bus)` (custom adapter injection). Wire all subsystems: Registry, Resolver, PolicyEngine, ContextBuilder, Providers, RetrievalEngine, CircuitBreakers. |
| 5.3.2 | Implement FabricFacade (main API) | TODO | k1/fabric/fabric.py | The CapabilityFabric class from `fabric_discussion.md` Section 15. Methods: `async execute(request) -> CapabilityResult`, `async execute_batch(requests, strategy: BatchStrategy) -> list[CapabilityResult]`. **execute_batch() semantics**: `BatchStrategy` enum: PARALLEL (all at once, default), SEQUENTIAL (ordered), DAG (dependency-aware). Partial failure handling: each request independent, failed requests return CapabilityResult.failure() without blocking others. Ordering guarantee: results list matches input order regardless of execution order. Backpressure: batch size capped at max_batch_size (default 50). Critical for Orchestrator DAG wave execution where multiple capabilities execute in a single wave. Enforces FAB-03 (stateless per-request), FAB-04 (circuit breaker timeout), FAB-09 (event emission). Wire OutputValidationPipeline (Epic 3.5) into execution path after provider returns. |
| 5.3.3 | Implement FabricRetrieval (retrieval API) | TODO | k1/fabric/fabric.py | Separate class for Role 1 API. Methods: `async discover_capabilities(domain, intent, safety_band, session_context, top_k) -> RetrievalResult`, `async find_relevant_prompts(intent, domain, safety_band, top_k) -> RetrievalResult`. |
| 5.3.4 | Implement CapabilityRegistryAPI | TODO | k1/fabric/fabric.py | Registry management API: register, unregister, lookup, update_availability, update_metrics, reload, health. Thin wrapper over core Registry. |
...
| Issue | Title | Status | Deliverable | Details |
|-------|-------|--------|-------------|---------|
| 5.4.1 | Define Fabric event types | TODO | k1/fabric/events/fabric_events.py | **Emitted**: k1.capability.invoked.v1, k1.capability.completed.v1, k1.capability.failed.v1, k1.fabric.capability.registered.v1, k1.fabric.capability.unregistered.v1, k1.fabric.capability.version.conflict.v1, k1.fabric.contract.validation.failed.v1, k1.fabric.output.validation.failed.v1, k1.fabric.learning.signal.v1 (payload: capability_name, provider_id, success, duration_ms, error_code, context_quality_score -- consumed by Learning Loop / K0 feedback pipeline P09), k1.fabric.provider.health.changed.v1, k1.fabric.capability.contract_updated.v1 (proactive gap detection -- emitted when capability contracts change, consumed by Orchestrator Workflow Engine for workflow invalidation), k1.fabric.pressure.warning.v1 (backpressure signal from mailbox). **Consumed**: k1.orchestration.step.execute.v1, k1.planner.discovery.request.v1, k1.fabric.provider.health.check.v1, k1.mcp.tool.discovered.v1 (from Orchestrator MCP Tool Discovery -- triggers programmatic registration). Event payloads as dataclasses with to_dict(). |
| 5.4.2 | Implement EventEmitter wrapper | TODO | k1/fabric/events/event_emitter.py | Wraps IEventPort. Enforces cognitive_trace_id on every event (FAB-09). Methods: `emit_invoked(request)`, `emit_completed(request, result)`, `emit_failed(request, error)`, `emit_registered(contract)`, `emit_unregistered(name)`. All carry trace_id + timestamp. |
| 5.4.3 | Wire events into execution pipeline | TODO | k1/fabric/fabric.py | Modify execute() to emit: (1) k1.capability.invoked.v1 before execution, (2) k1.capability.completed.v1 after success, (3) k1.capability.failed.v1 after failure. Modify register()/unregister() to emit registry events. Wire learning.signal.v1 after every result with full payload (capability_name, provider_id, success, duration_ms, error_code, context_quality_score). |
| 5.4.4 | Wire proactive gap detection | TODO | k1/fabric/events/event_emitter.py | Subscribe to registry change events (register, unregister, version conflict). On any contract change: emit `k1.fabric.capability.contract_updated.v1` with old_version, new_version, breaking_change flag. Orchestrator's Workflow Engine consumes this to invalidate affected frozen workflows and trigger re-validation. Also subscribe to `k1.mcp.tool.discovered.v1` from Orchestrator MCP Tool Discovery: on receipt, convert MCP tool manifest to CapabilityContract dict and call `register_from_dict()` (Issue 2.3.4). |
...
| Issue | Title | Status | Deliverable | Details |
|-------|-------|--------|-------------|---------|
| 6.3.1 | Test LOW tier execution flow | TODO | test_low_tier_flow.py | Full LOW tier: register tool contract -> fabric.execute(tool request) -> verify CapabilityResult success -> verify events emitted (invoked + completed) -> verify metrics updated. Test with MCP, WASM, Bridge provider types. |
| 6.3.2 | Test MEDIUM tier execution flow | TODO | test_medium_tier_flow.py | Register 2 tool contracts (calendar + notification). Execute both via fabric.execute_batch(). Verify parallel execution. Verify both results. Verify events for each. |
| 6.3.3 | Test HIGH tier retrieval + execution flow | TODO | test_high_tier_flow.py | Register 50+ contracts across domains. Discover via fabric.discover_capabilities(domain, intent). Verify top-K relevance. Then execute a discovered capability. Verify full pipeline. |
| 6.3.4 | Test agent spawn and execution | TODO | test_agent_execution.py | Register agent contract. Execute via fabric.execute() with agent capability name. Verify: agent lifecycle (PENDING -> WARMING -> ACTIVE -> result -> IDLE/TERMINATED), tool scoping enforced, context built from SessionState, delta emission on fact discovery, CapabilityResult returned. |
| 6.3.5 | Test safety band enforcement end-to-end | TODO | test_safety_band_e2e.py | Register capabilities with different safety_band_min (GREEN, AMBER, RED). Execute with GREEN user: verify GREEN-only caps succeed, AMBER+ rejected with access_denied. Test with AMBER user: GREEN + AMBER succeed, RED rejected. Verify FAB-06. |
| 6.3.6 | Test circuit breaker integration | TODO | test_circuit_breaker_integration.py | Register capability with a flaky provider (3 failures, then success). Verify: first 3 attempts fail, circuit opens, requests rejected without calling provider, after recovery period half-open, success closes circuit. Verify FAB-04 (30s timeout). |
...
| 6.4.3 | Test policies.contract.yaml compliance | TODO | test_policies_contract.py | Verify all SLI targets: registry lookup <1ms, retrieval <50ms, resolution <5ms, context build <50ms, overhead <100ms. Verify circuit breaker configs. Verify safety band ordering. |
| 6.4.4 | Test tool contract schema validation | TODO | test_tool_schema_validation.py | Test valid tool contracts pass schema validation. Test each invalid field raises specific error. Test all 12 validation rules. |
| 6.4.5 | Test agent contract schema validation | TODO | test_agent_schema_validation.py | Same as 6.4.4 for agent contracts. Additional: verify tools_granted references, llm_budget_tokens, max_tool_calls. |
...
| Issue | Title | Status | Deliverable | Details |
|-------|-------|--------|-------------|---------|
| 6.5.1 | Test Registry -> Retrieval flow | TODO | test_registry_retrieval.py | Register 100 contracts. Run 20 different retrieval queries. Verify: results come from registry, hard filters applied, soft ranking correct, top-K from registry pool. Hot-reload a contract, verify retrieval index updated. |
| 6.5.2 | Test Registry -> Resolution -> Execution flow | TODO | test_registry_resolution_execution.py | Register contract + provider. Execute capability. Verify: registry lookup correct, provider matched, policy evaluated, provider executed, result returned. Update availability to OFFLINE, verify execution fails with provider_offline. |
| 6.5.3 | Test Policy -> ContextBuilder -> Agent flow | TODO | test_policy_context_agent.py | Register agent with required_context including beliefs_active, persona. Policy passes. ContextBuilder reads SessionState. Agent receives correct context. Delta emitted on fact discovery. Full pipeline. |
| 6.5.4 | Test ModuleLoader -> Registry -> Retrieval update | TODO | test_loader_registry_retrieval.py | Start with empty registry. ModuleLoader scans fixture directory. Register all found contracts. Verify retrieval works on loaded contracts. Hot-reload: modify a YAML file, verify registry + retrieval index updated. |
...
| Issue | Title | Status | Deliverable | Details |
|-------|-------|--------|-------------|---------|
| 6.7.1 | Benchmark registry lookup | TODO | test_bench_registry.py | Registry with 1K, 10K, 100K contracts. Measure lookup latency: target <1ms P95. Measure list_by_domain, list_by_type latency. |
...
| Issue | Title | Status | Deliverable | Details |
|-------|-------|--------|-------------|---------|
| 7.1.1 | Define latency SLIs | TODO | SLI spec in policies.contract.yaml | **Registry lookup**: P95 <1ms. **Retrieval (10K)**: P95 <20ms. **Retrieval (100K)**: P95 <50ms. **Provider resolution**: P95 <5ms. **Context build (small)**: P95 <10ms. **Context build (large)**: P95 <50ms. **Full overhead**: P95 <100ms. **MCP local**: P95 <2s. **Agent execution**: P95 <10s. |
| 7.1.2 | Define throughput SLIs | TODO | SLI spec in policies.contract.yaml | **Concurrent executions**: 100 parallel. **Registry size**: 100K+ capabilities. **Retrieval QPS**: 1000 queries/sec. **Execution QPS**: 100 requests/sec. |
| 7.1.3 | Define availability SLOs | TODO | SLO spec in policies.contract.yaml | **Availability**: 99.9%. **Latency compliance**: 99.5% within P95 targets. **Retrieval accuracy**: 95% relevant results in Top-10. **Circuit breaker recovery**: <60s. |
...
| Issue | Title | Status | Deliverable | Details |
|-------|-------|--------|-------------|---------|
| 7.2.1 | Implement latency histograms | TODO | k1/fabric/metrics.py | Histograms: `fabric_execution_duration_seconds{capability_name, provider_type, tier}`, `fabric_retrieval_duration_seconds{query_type}`, `fabric_registry_lookup_duration_seconds`, `fabric_context_build_duration_seconds{capability_name}`, `fabric_policy_evaluation_duration_seconds`. Context managers: time_execution(), time_retrieval(), time_lookup(), time_context_build(). |
| 7.2.2 | Implement counters | TODO | k1/fabric/metrics.py | Counters: `fabric_executions_total{result: success/failure}`, `fabric_retrievals_total`, `fabric_registrations_total`, `fabric_circuit_breaker_trips_total{provider_id}`, `fabric_agent_spawns_total`, `fabric_retries_total{capability_name}`. |
| 7.2.3 | Implement gauges | TODO | k1/fabric/metrics.py | Gauges: `fabric_registry_size{capability_type}`, `fabric_circuit_breaker_state{provider_id, state}`, `fabric_agent_pool_size{contract_name}`, `fabric_active_executions`. |
...
| M4: Retrieval + Context + Agent Factory | 4 (4.1-4.4) | 15 |
| M5: Ports, Adapters & Standalone | 4 (5.1-5.4) | 21 |
| M6: Testing | 7 (6.1-6.7) | 45 |
