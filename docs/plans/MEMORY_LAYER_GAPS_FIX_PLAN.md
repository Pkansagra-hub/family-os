...
## 🧬 GAP-007: R4 Knowledge Graph Edge Enrichment (Long-term)

**Status**: Architectural Design Required
**Location**: `k0/pipelines/p03/phases/r4_kg_consolidator.py`
**Severity**: High (Root cause of R5 BGT-SM failure)
**Neurological Basis**: Hebbian Learning + Semantic Spreading Activation + Temporal Association

### GAP-007.1: Neurological Foundation — How Brains Build Associative Networks

The brain constructs rich associative networks through multiple complementary mechanisms. Each mechanism corresponds to a specific algorithm (not heuristic) that we must implement:

#### 7.1.1: Hebbian Learning — "Neurons That Fire Together Wire Together"

**Brain Process**: **Synaptic Plasticity** (Long-Term Potentiation)

**Neuroscience**:

- **LTP (Long-Term Potentiation)**: Repeated co-activation strengthens synaptic connections
- **Formula**: $\Delta w_{ij} = \eta \cdot x_i \cdot x_j$ (Hebb's Rule)
- **Temporal Window**: Co-activation within 10-50ms creates strongest bonds
- **Spike-Timing-Dependent Plasticity (STDP)**: Order matters — pre-synaptic firing before post-synaptic = strengthening

**Current R4 Implementation** (Already Exists):

```python
# k0/pipelines/p03/phases/r4_kg_consolidator.py:350
...
**Problem**: Only creates edges when entities **explicitly co-occur in same event**.

- 2056 events → only 8 edges (too sparse)
- Most events mention 1-2 entities only
- No implicit relationship inference

**Algorithm Enhancement**:

```python
# STDP-inspired temporal window association
class TemporalProximityEdgeCreator:
    """
    Creates edges between entities appearing in temporally proximate events.

    Neurological basis: STDP shows neurons firing within ~100ms window
    develop associative bonds. We scale this to event-level (minutes/hours).

    Algorithm (not heuristic):
        w_ij = η × exp(-|t_i - t_j| / τ) × (conf_i × conf_j)

    where:
        η = base learning rate (0.1)
        τ = temporal decay constant (3600000 ms = 1 hour)
        t_i, t_j = event timestamps
        conf_i, conf_j = entity extraction confidences
    """

    def __init__(self, temporal_window_ms: int = 3600000):  # 1 hour
        self.tau = temporal_window_ms
        self.eta = 0.1
    ...
    def compute_edge_weight(
        ...
        """
        Compute associative strength using exponential temporal decay.

        Returns:
            Edge weight in [0, 1]. Values < 0.01 should not create edges.
        """
        dt = abs(event_a_time - event_b_time)
        temporal_factor = math.exp(-dt / self.tau)
        confidence_factor = entity_a.confidence * entity_b.confidence

        return self.eta * temporal_factor * confidence_factor
```

#### 7.1.2: Semantic Spreading Activation — "Related Concepts Prime Each Other"

**Brain Process**: **Semantic Priming** (Temporal Lobe Networks)

**Neuroscience**:

- **Spreading Activation**: Activating one concept automatically partially activates related concepts
- **Meyer & Schvaneveldt (1971)**: "Doctor" primes "Nurse" — faster recognition due to semantic link
- **Collins & Loftus (1975)**: Semantic networks have weighted links based on associative strength
- **Formula**: $A_j(t+1) = \sum_i w_{ij} \cdot A_i(t) \cdot (1 - d)$ where $d$ = decay

**Algorithm**: **Semantic Similarity Edge Creator**

```python
class SemanticSimilarityEdgeCreator:
    """
    Creates edges between semantically similar entities based on embedding distance.

    Neurological basis: Semantic memory stores concepts in high-dimensional
    space where distance correlates with associative strength.

    Algorithm:
        similarity = 1 - cosine_distance(emb_a, emb_b)
        edge_weight = max(0, similarity - threshold) / (1 - threshold)

    where threshold = 0.7 (only highly similar entities get edges)

    This is NOT a heuristic because:
        1. Cosine similarity is a well-defined metric
        2. Threshold is derived from word2vec/BERT semantic similarity studies
        3. Output weight is a normalized probability
    """

    SIMILARITY_THRESHOLD = 0.7  # Derived from NLP literature
    ...
    def compute_edge_weight(
        ...
        """
        Compute semantic association strength from embedding similarity.

        Returns:
            Normalized edge weight in [0, 1]. Zero if below threshold.
        """
        # Cosine similarity
        ...
        return (similarity - self.SIMILARITY_THRESHOLD) / (1 - self.SIMILARITY_THRESHOLD)

    def create_edges_for_entity(
        self,
        entity: KGEntity,
        ...
        """
        Find top-k most semantically similar entities and create edges.

        Uses efficient nearest-neighbor search via FAISS.
        """
        similarities = []
        for other in all_entities:
            ...
            if other.entity_type != entity.entity_type:
                continue  # Only connect same-type entities

            weight = self.compute_edge_weight(entity.embedding, other.embedding)
            if weight > 0:
                similarities.append((other.entity_id, weight))

        # Sort and take top-k
        similarities.sort(key=lambda x: x[1], reverse=True)

        return [
            (entity.entity_id, other_id, weight)
            for other_id, weight in similarities[:max_edges]
        ]
```

#### 7.1.3: Relational Inference — "If A→B and B→C, Then A↔C"

**Brain Process**: **Transitive Inference** (Hippocampus + Prefrontal Cortex)

**Neuroscience**:

- **Hippocampal Relational Memory**: Encodes relationships between items, not just items themselves
- **Transitive Inference**: Rats and humans infer A>E from training A>B, B>C, C>D, D>E
- **Eichenbaum (2004)**: Hippocampus represents "relational networks" enabling inference
- **Formula**: $P(A \rightarrow C) = P(A \rightarrow B) \times P(B \rightarrow C)$ (probabilistic transitivity)

**Algorithm**: **Transitive Closure Edge Creator**

```python
class TransitiveClosureEdgeCreator:
    """
    Creates inferred edges through transitive inference on existing graph.

    Neurological basis: Hippocampus encodes relational structure enabling
    transitive inference. A→B→C implies weaker A→C connection.

    Algorithm (probabilistic transitivity):
        w_AC = w_AB × w_BC × decay_factor

    where decay_factor = 0.5 (each hop halves confidence)

    Constraints:
        - Max path length: 2 (A→B→C, not A→B→C→D)
        - Min weight threshold: 0.1 (no weak inferences)
        - Same relation type only (FAMILY→FAMILY, not FAMILY→COLLEAGUE)
    """
    ...
    def infer_transitive_edges(
        self,
        graph: Dict[str, Dict[str, float]],  # source_id -> {target_id: weight}
        relation_types: Dict[Tuple[str, str], str],  # (source, target) -> relation
    ) -> List[Tuple[str, str, float, str]]:
        """
        Compute transitive closure with probabilistic weight decay.

        Returns:
            List of (source_id, target_id, weight, relation_type) for inferred edges.
        """
        inferred = []

        for a in graph:
            for b, w_ab in graph[a].items():
                if b not in graph:
                    continue

                for c, w_bc in graph[b].items():
                    if c == a:  # No self-loops via transitive
                        continue
                    if c in graph.get(a, {}):  # Direct edge already exists
                        continue

                    # Check relation type compatibility
                    rel_ab = relation_types.get((a, b))
                    rel_bc = relation_types.get((b, c))
                    if rel_ab != rel_bc:
                        continue  # Only same-type transitivity

                    # Compute transitive weight
                    w_ac = w_ab * w_bc * self.DECAY_FACTOR

                    if w_ac >= self.MIN_WEIGHT:
                        inferred.append((a, c, w_ac, rel_ab))

        return inferred
```

#### 7.1.4: Causal Inference — "Temporal Precedence Implies Causation"

**Brain Process**: **Predictive Coding** (Prefrontal Cortex)

**Neuroscience**:

- **Forward Models**: Brain predicts future states from current actions
- **Granger Causality**: X "Granger-causes" Y if past X improves prediction of Y
- **Temporal Asymmetry**: Causes precede effects (core assumption of causal cognition)
- **Formula**: $GC_{X \rightarrow Y} = \log \frac{Var(Y | Y_{past})}{Var(Y | Y_{past}, X_{past})}$

**Current R4 Implementation** (Exists but Underutilized):

```python
# k0/pipelines/p03/phases/r4_kg_consolidator.py:1564
self._granger_causality = GrangerCausalityInference()
```

**Problem**: Requires 5+ observations (`granger_min_observations`) but most entity pairs have < 2.

**Algorithm Enhancement**: **Lower Threshold + Bayesian Prior**

```python
class BayesianCausalInference:
    """
    Causal inference with Bayesian priors for sparse data.

    Neurological basis: Brain uses prior knowledge to infer causality
    even with limited observations (one-shot causal learning).

    Algorithm:
        P(A→B | data) = P(data | A→B) × P(A→B) / P(data)

    Prior P(A→B) based on:
        - Semantic compatibility (can A cause B?)
        - Temporal ordering (does A typically precede B?)
        - Entity type constraints (PERSON can cause ACTION)

    This overcomes sparse data by encoding domain knowledge as priors.
    """

    # Entity type causality matrix (can source_type cause target_type?)
    CAUSALITY_PRIORS = {
        ("PERSON", "ACTION"): 0.8,    # People cause actions
        ("PERSON", "EVENT"): 0.7,     # People cause events
        ("LOCATION", "EVENT"): 0.3,   # Locations enable events (weaker)
        ("TIME", "EVENT"): 0.6,       # Time constrains events
        ("ACTION", "EMOTION"): 0.7,   # Actions cause emotions
        ("PERSON", "PERSON"): 0.4,    # Social influence (moderate)
    }
    ...
    def compute_causal_probability(
        ...
        source_type: str,
        target_type: str,
        temporal_precedence: float,  # P(source before target)
        co_occurrence_count: int,
    ) -> float:
        """
        Compute posterior probability of causal relationship.

        Uses Bayesian update with entity-type-based priors.
        """
        # Prior from entity type compatibility
        prior = self.CAUSALITY_PRIORS.get((source_type, target_type), 0.2)

        # Likelihood from temporal precedence
        # Strong precedence (>0.8) = likely causal
        likelihood = temporal_precedence if temporal_precedence > 0.5 else 0.1

        # Evidence strength from observation count
        # More observations = more confident (log scaling)
        evidence_weight = min(1.0, math.log1p(co_occurrence_count) / 3.0)

        # Bayesian posterior (simplified)
        posterior = prior * (1 - evidence_weight) + likelihood * evidence_weight

        return min(0.95, posterior)  # Cap at 95% confidence
```

#### 7.1.5: Contextual Association — "Context Determines Meaning"

**Brain Process**: **Context-Dependent Memory** (Hippocampus + Prefrontal Cortex)

**Neuroscience**:

- **State-Dependent Learning**: Memory recall improved when context matches encoding
- **Context Reinstatement**: Returning to same location improves memory retrieval
- **Pattern Separation/Completion**: Hippocampus uses context to disambiguate similar memories
- **Anderson (1983)**: ACT-R model — context modulates spreading activation

**Algorithm**: **Contextual Edge Creator**

```python
class ContextualEdgeCreator:
    """
    Creates edges between entities sharing contextual features.

    Neurological basis: Hippocampus encodes context (time, place, emotional state)
    as part of memory trace. Shared context creates associative bonds.

    Algorithm:
        context_similarity = jaccard(context_a, context_b)
        edge_weight = context_similarity × base_weight

    Context features:
        - Location (place_id)
        - Time bucket (morning/afternoon/evening)
        - Activity type (work/leisure/social)
        - Emotional valence (positive/negative/neutral)
        - Social context (alone/with_family/with_colleagues)
    """

    def extract_context_vector(self, event: P03EventState) -> Set[str]:
        """
        Extract categorical context features from event.
        """
        context = set()

        # Temporal context
        hour = (event.timestamp // 3600000) % 24
        if 5 <= hour < 12:
            context.add("time:morning")
        elif 12 <= hour < 17:
            context.add("time:afternoon")
        elif 17 <= hour < 21:
            context.add("time:evening")
        else:
            context.add("time:night")

        # Day of week
        day = (event.timestamp // 86400000) % 7
        if day < 5:
            context.add("day:weekday")
        else:
            context.add("day:weekend")

        # Activity type (from UltraBERT)
        if event.activity_type:
            context.add(f"activity:{event.activity_type}")

        # Location (if available)
        if event.location_id:
            context.add(f"location:{event.location_id}")

        # Emotional valence
        if event.sentiment_score is not None:
            if event.sentiment_score > 0.3:
                context.add("emotion:positive")
            elif event.sentiment_score < -0.3:
                context.add("emotion:negative")
            else:
                context.add("emotion:neutral")
    ...
    def compute_context_similarity(
        ...
        """
        Compute Jaccard similarity between context vectors.
        """
        if not context_a or not context_b:
            return 0.0

        intersection = len(context_a & context_b)
        union = len(context_a | context_b)

        return intersection / union if union > 0 else 0.0

    def create_contextual_edges(
        self,
        entity_contexts: Dict[str, List[Set[str]]],  # entity_id -> list of contexts
        min_similarity: float = 0.5,
    ) -> List[Tuple[str, str, float]]:
        """
        Create edges between entities with high context overlap.

        For each entity pair, computes average context similarity
        across all co-occurring events.
        """
        edges = []
        entity_ids = list(entity_contexts.keys())

        for i, entity_a in enumerate(entity_ids):
            for entity_b in entity_ids[i + 1:]:
                contexts_a = entity_contexts[entity_a]
                contexts_b = entity_contexts[entity_b]

                # Compute pairwise context similarities
                similarities = []
                for ctx_a in contexts_a:
                    for ctx_b in contexts_b:
                        sim = self.compute_context_similarity(ctx_a, ctx_b)
                        if sim > 0:
                ...
                if similarities:
                    ...
                    if avg_similarity >= min_similarity:
                        edges.append((entity_a, entity_b, avg_similarity))

        return edges
```

### GAP-007.2: Implementation Architecture

**Phase 1: Edge Enrichment Algorithms** (Design + Implement)

| Algorithm | Neurological Basis | Expected Edge Increase | Effort |
|-----------|-------------------|------------------------|--------|
| TemporalProximityEdgeCreator | STDP | +50-100 edges | 2 days |
| SemanticSimilarityEdgeCreator | Spreading Activation | +100-200 edges | 3 days |
| TransitiveClosureEdgeCreator | Transitive Inference | +30-50 edges | 2 days |
| BayesianCausalInference | Predictive Coding | +20-40 causal edges | 2 days |
| ContextualEdgeCreator | Context-Dependent Memory | +50-100 edges | 2 days |

**Total Expected**: 250-490 new edges → **1.7-3.3 edges/entity** (vs current 0.054)

**Phase 2: R4 Integration**

```python
# Enhanced R4 flow:
async def run(self, envelope: P03BatchEnvelope, ctx: P03RunnerContext):
    # Step 1: Extract entities (existing)
    entities = await self._extract_entities(envelope.events)

    # Step 2: Explicit co-occurrence edges (existing Hebbian)
    co_occurrence_edges = await self._discover_relationships(entities, events)

    # Step 3: NEW — Temporal proximity edges
    temporal_edges = await self._create_temporal_proximity_edges(entities, events)

    # Step 4: NEW — Semantic similarity edges
    semantic_edges = await self._create_semantic_similarity_edges(entities)

    # Step 5: NEW — Contextual edges
    contextual_edges = await self._create_contextual_edges(entities, events)

    # Step 6: Merge all edge sources
    all_edges = self._merge_edge_sources(
        co_occurrence_edges,
        temporal_edges,
        semantic_edges,
        contextual_edges,
    ...
    # Step 7: NEW — Transitive closure
    transitive_edges = self._compute_transitive_closure(all_edges)
    all_edges.extend(transitive_edges)

    # Step 8: Causal inference (enhanced)
    causal_edges = await self._infer_causal_relationships(all_edges, entities)

    # Step 9: Stage for R6/R7
    envelope.phases.r4_new_edges = all_edges
```

**Phase 3: Edge Weight Normalization**

```python
class EdgeWeightNormalizer:
    """
    Normalizes edge weights from multiple sources to common scale.

    Neurological basis: Synaptic weights are normalized through
    homeostatic plasticity to prevent runaway excitation.

    Algorithm:
        1. Compute mean and std for each edge source
        2. Z-score normalize within source
        3. Map to [0, 1] using sigmoid
        4. Combine multiple edges with max (not sum) to prevent inflation
    """

    def normalize_and_merge(
        self,
        edge_sources: Dict[str, List[Tuple[str, str, float]]],
    ) -> Dict[Tuple[str, str], float]:
        """
        Normalize weights and merge edges from multiple sources.

        If same edge appears in multiple sources, take maximum weight.
        """
        merged = {}

        for source_name, edges in edge_sources.items():
            if not edges:
                continue
            ...
            std_w = max(std_w, 0.01)  # Avoid division by zero

            for source_id, target_id, weight in edges:
                # Z-score normalize
                z = (weight - mean_w) / std_w
                # Sigmoid to [0, 1]
                normalized = 1 / (1 + math.exp(-z))

                key = (source_id, target_id)
                if key not in merged or normalized > merged[key]:
                    merged[key] = normalized

        return merged
```

### GAP-007.3: Validation — How to Know Algorithms Work

**Metric 1: Edge Density**

```sql
-- Before: 0.054 edges/entity
-- Target: 1.0-2.0 edges/entity
SELECT COUNT(*)::float / (SELECT COUNT(*) FROM st_kg_dom) as edges_per_entity
FROM st_kg_edges;
```

**Metric 2: Graph Connectivity**

```sql
-- Before: 135/147 isolated nodes (91.8%)
-- Target: <20% isolated nodes
SELECT COUNT(*) * 100.0 / (SELECT COUNT(*) FROM st_kg_dom) as isolated_pct
FROM st_kg_dom e
WHERE NOT EXISTS (
  SELECT 1 FROM st_kg_edges
  WHERE source_entity_id = e.entity_id OR target_entity_id = e.entity_id
);
```

**Metric 3: BGT-SM Insight Generation**

```sql
-- Before: 0 insights
-- Target: 5-20 insights per cycle
SELECT COUNT(*) FROM st_sem WHERE pattern_type = 'INSIGHT';
```

**Metric 4: Edge Type Distribution**

```sql
-- Should have diverse edge types, not just RELATED_TO
...
**Metric 5: Algorithm Attribution**

```sql
-- Track which algorithm created each edge (add source_algorithm column)
SELECT source_algorithm, COUNT(*) as edges_created
FROM st_kg_edges
WHERE source_algorithm IS NOT NULL
GROUP BY source_algorithm;
-- Expected: co_occurrence, temporal_proximity, semantic_similarity,
--           contextual, transitive, causal
```

### GAP-007.4: Why Algorithms, Not Heuristics

| Approach | Heuristic Example | Algorithm Example |
|----------|-------------------|-------------------|
| Temporal Association | "If events are within 1 hour, connect" | STDP-based exponential decay: $w = \eta \cdot e^{-\Delta t / \tau}$ |
| Semantic Similarity | "Connect if names are similar" | Cosine similarity in embedding space with learned threshold |
| Transitive Closure | "If A→B→C, then A→C" | Probabilistic transitivity with confidence decay |
| Causal Inference | "Earlier event causes later" | Granger causality with Bayesian priors |
| Context Matching | "Same location = connected" | Jaccard similarity over multi-dimensional context vectors |

**Key Differences**:

1. **Heuristics** are rules of thumb with magic numbers
2. **Algorithms** have mathematical foundations and learned parameters
3. **Heuristics** fail silently on edge cases
4. **Algorithms** have defined behavior for all inputs
5. **Heuristics** can't be validated
6. **Algorithms** have measurable precision/recall

### GAP-007.5: Neurological Validation Criteria

**The enhanced edge creation mimics these brain processes**:

| Brain Process | System Equivalent | Validation |
|---------------|-------------------|------------|
| LTP (Long-Term Potentiation) | Hebbian weight updates | Edge weights increase with repetition |
| Spreading Activation | Semantic similarity edges | Priming effect on related queries |
| Transitive Inference | Path-based edge creation | A→C inferrable from A→B→C |
| Predictive Coding | Causal edge direction | Temporal precedence detected |
| Context-Dependent Memory | Context similarity edges | Same-context entities clustered |
| Synaptic Homeostasis | Weight normalization | No runaway weights, bounded [0, 1] |

### GAP-007.6: Estimated Effort

| Phase | Task | Effort |
|-------|------|--------|
| Design | ADR + Algorithm specs | 2 days |
| Implementation | TemporalProximityEdgeCreator | 2 days |
| Implementation | SemanticSimilarityEdgeCreator | 3 days |
| Implementation | TransitiveClosureEdgeCreator | 2 days |
| Implementation | BayesianCausalInference | 2 days |
| Implementation | ContextualEdgeCreator | 2 days |
| Implementation | EdgeWeightNormalizer | 1 day |
| Integration | R4 pipeline changes | 2 days |
| Integration | Database schema (source_algorithm column) | 1 day |
| Testing | Unit + Integration tests | 3 days |
| Validation | Run on historical data, verify metrics | 2 days |
| **Total** | | **22 days (~4.5 weeks)** |

### GAP-007.7: Success Criteria

**Graph Density**:

- Before: 8 edges, 0.054 edges/entity
- After: 250-500 edges, 1.7-3.3 edges/entity
...
- Before: 0 insights generated
- After: 5-20 insights per consolidation cycle

**Edge Algorithm Attribution**:
