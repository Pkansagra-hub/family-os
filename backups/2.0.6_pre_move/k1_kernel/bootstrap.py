...
try:
    ...
    _root = Path(__file__).resolve().parents[2]  # familyos/
    _env_candidates = [
        _root / ".env",
...
from k1.concierge.config.kernel import KernelConfig  # single source of truth
...
async def start_kernel(config: KernelConfig | None = None) -> KernelRuntime:
    ...
    # --- Bus infrastructure (was: poc.k1_poc.main.boot()) ---
    # Issue 2.0.3: Direct factory calls, no POC dependency.
    # Issue 2.0.5: Middleware wiring via shared build_middleware_chain().
    ...
    _middleware = build_middleware_chain(cfg.bus)

    _mailbox_cfg = MailboxConfig(
        capacity=cfg.bus.mailbox_capacity,
        priority_wfq=cfg.bus.priority_wfq,
    )

    if cfg.ordered_bus:
        bus = BusFactory.create_local_ordered(
            capture=cfg.capture_bus,
            timeout_ms=cfg.bus.gap_timeout_ms,
            middleware=_middleware,
        )
    else:
        bus = BusFactory.create_local(capture=cfg.capture_bus, middleware=_middleware)
    router = BusFactory.create_mailbox_router()
    adapter = SessionBusAdapter(bus)
    front_mailbox = router.register("front_half", config=_mailbox_cfg)
    back_mailbox = router.register("back_half", config=_mailbox_cfg)
    ...
    if cfg.phase1_pipeline == "ultrabert":
        ...
        _adapter = K1UltraBERTAdapter(warmup=cfg.phase1_warmup_on_startup)
    ...
    if cfg.enable_delta:
        ...
        runtime.delta_aggregator = DeltaAggregator(
            flush_fn=applicator.apply, batch_window_ms=cfg.delta_batch_window_ms
        )
...
async def _mailbox_consumer(runtime: KernelRuntime) -> None:
    ...
    poll_interval = runtime.config.poll_interval_s
    dedup_limit = runtime.config.dedup_cache_size
...
def _create_session_state(cfg: KernelConfig) -> Any:
    from k1.sessionstate.public_types import SessionStateFactory
...
# ---------------------------------------------------------------------------
from k1.concierge.factory import (  # noqa: E402
