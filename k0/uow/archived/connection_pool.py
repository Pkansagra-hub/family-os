...
.. deprecated:: 2.0.0
    This entire module is deprecated. Use:
    - `k0.db.pool` for PostgreSQL connection pool management
    - `k0.db.connection.connection_scope()` for async connection context managers

    SQLite support is retained ONLY for rollback scenarios.
    The UnitOfWork class now uses k0.db.connection directly.
"""
...
# Mark entire module as deprecated on import
warnings.warn(
    "k0.uow.connection_pool is deprecated. Use k0.db.connection for PostgreSQL.",
    DeprecationWarning,
    stacklevel=2,
)
...
def configure_pool(
    ...
    *,
    ...
    .. deprecated:: 2.0.0
        ...
        This SQLite pool is retained for rollback scenarios only.
    """
    warnings.warn(
        "SQLite connection pool is deprecated. Use k0.db.pool.configure_pool() for PostgreSQL.",
        DeprecationWarning,
        stacklevel=2,
    )
...
def shutdown_pool() -> None:
    ...
    .. deprecated:: 2.0.0
        Use `k0.db.pool.shutdown_pool()` for PostgreSQL instead.
    """
    warnings.warn(
        "SQLite shutdown_pool is deprecated. Use k0.db.pool.shutdown_pool() for PostgreSQL.",
        DeprecationWarning,
        stacklevel=2,
    )
...
def get_pool() -> SQLiteConnectionPool:
    ...
    .. deprecated:: 2.0.0
        Use `k0.db.pool.get_pool()` for PostgreSQL instead.
    """
    warnings.warn(
        "SQLite get_pool is deprecated. Use k0.db.pool.get_pool() for PostgreSQL.",
        DeprecationWarning,
        stacklevel=2,
    )
...
def connection_scope(*, timeout: float | None = None) -> Iterator[sqlite3.Connection]:
    ...
    .. deprecated:: 2.0.0
        Use `k0.db.connection.connection_scope()` for PostgreSQL instead.
    """
    warnings.warn(
        "SQLite connection_scope is deprecated. Use k0.db.connection.connection_scope() for PostgreSQL.",
        DeprecationWarning,
        stacklevel=2,
    )
...
# Migration shims - redirect to new PostgreSQL pool
async def get_async_pool():
    """Get asyncpg pool (migration shim).

    Use this during migration to access the PostgreSQL pool
    from code that currently uses SQLite.

    Returns:
        AsyncPgPool instance from k0.db.pool
    """
    from k0.db.pool import get_pool as get_pg_pool

    return get_pg_pool()


async def async_connection_scope():
    """Get async PostgreSQL connection (migration shim).

    Use this during migration as an async replacement for
    the sync connection_scope().

    Returns:
        Async context manager yielding asyncpg.Connection
    """
    from k0.db.connection import connection_scope as pg_connection_scope

    return pg_connection_scope()
