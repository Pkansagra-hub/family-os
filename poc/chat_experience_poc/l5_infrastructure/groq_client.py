...
Provides async wrapper around Groq API with:
...
References:
- config/groq_config.py (model, temperature, token limits)
...
class GroqClient:
    ...
    async def complete(
        ...
        if requested_model in FALLBACK_MODELS:
            # Add remaining models after the requested one
            idx = FALLBACK_MODELS.index(requested_model)
            models_to_try.extend(FALLBACK_MODELS[idx + 1 :])
        else:
            # Requested model not in fallback list, try all fallbacks
        ...
        for model_idx, current_model in enumerate(models_to_try):
            ...
            while attempt < MAX_RETRIES:
                try:
                    ...
                    self.error_count += 1
                    error_msg = (
                        f"Timeout/connection error on {current_model} (attempt {attempt}): {str(e)}"
                    )
                    all_errors.append(error_msg)
                    ...
                    # Non-retryable errors (invalid API key, bad request, etc.)
                    self.error_count += 1
