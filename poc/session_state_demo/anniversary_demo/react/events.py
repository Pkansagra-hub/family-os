...
display layer.  The display layer implements ``LoopEventHandler`` to
render these events as they happen.

Separation of concerns:
    - ``react/loop.py`` emits events via ``_emit()``
    - The demo runner receives events via a handler implementation
    - This module defines the shared contract between them.

Ported from ``concierge_fsm_poc/react/events.py``.
"""
...
class LoopEventHandler(Protocol):
    ...
    Implementations must be synchronous -- display rendering runs
