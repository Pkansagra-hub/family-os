...
class TestBackTopicRouting:
    ...
    async def test_task_resume_routes_to_resume_handler(self) -> None:
        ...
        with (
            patch(
                "poc.k1_poc.actors.back.back_resume_handler", new_callable=AsyncMock
            ) as mock_resume,
            patch("poc.k1_poc.actors.back.back_handler", new_callable=AsyncMock) as mock_back,
        ):
    ...
    async def test_task_cancel_routes_to_cancel_handler(self) -> None:
        ...
        with (
            patch("poc.k1_poc.actors.back.back_cancel_handler") as mock_cancel,
            patch("poc.k1_poc.actors.back.back_handler", new_callable=AsyncMock) as mock_back,
        ):
    ...
    async def test_cancel_handler_receives_cancel_token(self) -> None:
        ...
        with patch("poc.k1_poc.actors.back.back_cancel_handler") as mock_cancel:
            await route_back_envelope(envelope=env, **deps)
...
class TestParallelToolSafety:
    ...
    async def test_parallel_tools_run_concurrently(self) -> None:
        ...
        with patch("poc.k1_poc.react.loop.get_config") as mock_config:
            mock_cfg = MagicMock()
