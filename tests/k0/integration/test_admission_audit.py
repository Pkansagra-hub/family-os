...
def test_admission_middleware_persists_receipts_and_emits_telemetry_signals() -> None:
    ...
    async def admit_endpoint(request: Request) -> JSONResponse:
        ...
        envelope["tenant_id"] = "mutated"  # ensure defensive copy for middleware processing
    ...
    # NOTE: Receipts are NOT saved by admission middleware.
    # Receipt persistence happens inside UnitOfWork transaction (command.py:769)
    # The middleware only emits observability events and metrics.
    # See: k0/kernel/app.py lines ~1289-1294 for explanation
    assert len(receipt_store.saved) == 0  # Middleware does not save receipts

    decision_events = [
        evt for evt in observability_emitter.events if evt.get("event") == "observe_policy_decision"
    ]
...
def test_admission_middleware_captures_errors_when_downstream_handlers_fail() -> None:
    ...
    async def deny_endpoint(request: Request) -> JSONResponse:
        decision = PolicyDecision(admit=False, obligations=(), deny_reason="ROLE_FORBIDDEN")
        envelope: Dict[str, Any] = {
    ...
    decision_events = [
        evt for evt in observability_emitter.events if evt.get("event") == "command_policy_decision"
    ]
    assert len(decision_events) == 1
    event = decision_events[0]
