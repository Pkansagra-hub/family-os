"""Unit tests for ``SpaceGraphService`` (M1.E1.I2)."""
...
from k1.selfmodel.service.space_graph import _adjacent_member_ids
...
def test_get_view_unknown_family_returns_empty_snapshot() -> None:
    bundle = build_bundle()
    res = bundle.space_graph.get_view("a1", "fs:nonexistent")
...
def test_single_actor_household_has_no_relations() -> None:
    ...
    res = bundle.space_graph.get_view("a1", DEFAULT_FAMILY_SPACE)
    assert res.snapshot.relations == ()
...
def test_only_adjacent_edges_returned() -> None:
    ...
    bundle = build_bundle(family=fam)
    res = bundle.space_graph.get_view("g1", DEFAULT_FAMILY_SPACE)
...
def test_get_view_requires_actor_and_family() -> None:
    ...
    with pytest.raises(ValueError):
        bundle.space_graph.get_view("", "fs1")
    with pytest.raises(ValueError):
        bundle.space_graph.get_view("a1", "")
...
def test_get_member_lookup() -> None:
    ...
    bundle = build_bundle(family=fam)
    g = bundle.space_graph.get_member(DEFAULT_FAMILY_SPACE, "g1")
    assert g is not None and g.role == "guardian"
    assert bundle.space_graph.get_member(DEFAULT_FAMILY_SPACE, "ghost") is None
    assert bundle.space_graph.get_member("fs:nonexistent", "g1") is None


def test_freshness_helper() -> None:
    fam = make_family(members=(make_member("a1", role="guardian"),))
    bundle = build_bundle(family=fam)
    assert bundle.space_graph.freshness(DEFAULT_FAMILY_SPACE).value == "fresh"
    bundle.store.mark_stale(f"space:{DEFAULT_FAMILY_SPACE}")
    assert bundle.space_graph.freshness(DEFAULT_FAMILY_SPACE).value == "stale"
