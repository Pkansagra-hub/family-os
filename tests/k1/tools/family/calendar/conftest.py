"""Shared fixtures for the family Calendar adapter tests."""
...
def svc(tmp_path: Path):
    """Construct a ``CalendarToolService`` against an isolated SQLite store."""

    store = K1FamilyStore(str(tmp_path / "cal.db"))
    pub = RecordingSsePublisher()
    emitter = EventEmitter(pub)
    idem = IdempotencyStore(store.conn)
    service = CalendarToolService(store.conn, emitter, default_policy(), idem)
    try:
