"""Shared fixtures for the Chores adapter tests."""
...
def svc(tmp_path: Path):
    store = K1FamilyStore(str(tmp_path / "chores.db"))
    pub = RecordingSsePublisher()
    emitter = EventEmitter(pub)
    idem = IdempotencyStore(store.conn)
    service = ChoresToolService(store.conn, emitter, default_policy(), idem)
    try:
