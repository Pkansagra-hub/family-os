...
from k1.concierge.delta.aggregator import DeltaAggregator, DeltaBatch
from k1.concierge.delta.applicator import ApplyResult, DeltaApplicator
from k1.concierge.delta.emitters import VALID_TASK_STATUSES, emit_artifact, emit_task_state_change
from k1.concierge.delta.overflow import (
    HISTORY_WINDOW_SIZE,
    HOT_BUDGET_TOTAL,
    SECTION_BUDGETS,
...
class TestDeltaPackageImportability:
    ...
    EXPECTED_MODULES = [
        "k1.concierge.delta.session_delta",
        "k1.concierge.delta.topics",
        "k1.concierge.delta.emitters",
        "k1.concierge.delta.aggregator",
        "k1.concierge.delta.applicator",
        "k1.concierge.delta.writer_registry",
        "k1.concierge.delta.snapshot_reader",
        "k1.concierge.delta.overflow",
    ]
    ...
    def test_top_level_package_importable(self) -> None:
        mod = importlib.import_module("k1.concierge.delta")
        assert mod is not None
...
class TestDeltaPackageExports:
    ...
    def test_export_count_is_24(self) -> None:
        import k1.concierge.delta as delta
...
class TestDeltaPackageStructure:
    ...
    def test_submodule_count(self) -> None:
        ...
        for m in modules:
            mod = importlib.import_module(f"k1.concierge.delta.{m}")
            assert mod is not None
