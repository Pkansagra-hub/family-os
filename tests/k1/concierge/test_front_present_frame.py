...
def test_present_typed_frame_excludes_worker_and_capability_leaks() -> None:
    frame = BackResultFrame(
        task_id="task-1",
        facts=[
            {
        ...
        raw_final_answer=(
            "REACT_LOOP_DEGENERATE tool.read.calendar.list_events "
            '{"capability_name":"tool.read.family_tasks.list_tasks","member_id":"riley"}'
        ),
    )
        ...
        _env(
            {
                ...
                "final_answer": "Legacy worker prose should not drive presentation.",
                "frame": frame.to_dict(),
            }
        ),
        ss=None,
    ...
    assert "Pack lunch" in prompt_visible
    assert "Soccer at 6 PM" in prompt_visible
    for leak in (
        "_frame_type",
        "capability_name",
        "tool.read",
        ...
        assert leak not in prompt_visible
